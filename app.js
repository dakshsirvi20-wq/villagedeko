...
async function persistVillageToFirebase(data){
 ...
 // Upload photo/video fields for every Village Hub row except Ek Din Gaon Mein.
 // Each uploaded media URL stays inside the same row, so one village never borrows another village's media.
 for(const type of ['explore','experience','food','journey','people','culture','package']){
  ...
  for(let i=0;i<data[type].length;i++){
   ...
   for(let j=0;j<defs.length;j++){
    if(defs[j]?.[1]!=='image'&&defs[j]?.[1]!=='video')continue;
...
function rowMediaMarkup(row,photoIndex,videoIndex,alt='Village media'){const photo=row?.[photoIndex]||'',video=row?.[videoIndex]||'';if(!photo&&!video)return '';return `<div class="hub-row-media">${photo?`<img src="${esc(photo)}" alt="${esc(alt)}">`:''}${video?`<video src="${esc(video)}" controls playsinline preload="metadata"></video>`:''}</div>`;}
...
const listingTemplates={
 ...
 experience:[['Experience name','text','e.g. Farm Visit'],['Category','select',['Activities','Food','Culture','Nature','Farming','Craft','Other']],['Duration','text','e.g. 2 Hours'],['Price per person','number','₹'],['Description','textarea','What will visitors do?'],['Experience photo','image',''],['Experience video','video','']],
 day:[['Time','text','e.g. 05:30 AM'],['Activity','text','e.g. Gaon ki subah'],['Description','textarea','What happens at this time?']],
 food:[['Dish name','text','e.g. Bajre ki roti'],['Type','text','e.g. Breakfast / Lunch'],['Description','textarea','What makes this food local?'],['Dish photo','image',''],['Dish video','video','']],
 journey:[['Stage name','text','e.g. Beej / Khet / Katai'],['From','text','Where does this stage start?'],['To','text','Where does it go next?'],['Description','textarea','Explain this part of the food journey.'],['Stage photo','image',''],['Stage video','video','']],
 people:[['Person name','text','e.g. Gopal Singh'],['Role','text','e.g. Farmer / Artisan'],['Story title','text','e.g. 40 years of farming'],['Story','textarea','Tell their story.'],['Person / story photo','image',''],['Person / story video','video','']],
...
function listingFieldHtml(def){const [label,type,extra]=def;if(type==='select')return `<label>${label}<select>${extra.map(x=>`<option>${x}</option>`).join('')}</select></label>`;if(type==='image'||type==='video')return `<label class="full upload-card">${label}<input type="file" accept="${type==='video'?'video/*':'image/*'}" data-file-field="1"><span>${type==='video'?'▶':'▧'}</span><strong>Choose ${type}</strong><small>Optional ${type} for this exact village item</small></label>`;return `<label>${label}<${type==='textarea'?'textarea':'input'} ${type!=='textarea'?`type="${type}"`:''} placeholder="${extra||''}" ${type==='textarea'?'rows="3"':''}></${type==='textarea'?'textarea':'input'}></label>`}
...
async function saveVillageListing(){const f=document.getElementById('village-listing-form');if(!f.checkValidity()){f.reportValidity();return}const data=getListingData();
 const experienceRows=[...document.querySelectorAll('#experience-repeat .repeat-card')];
 for(let i=0;i<experienceRows.length;i++){const row=experienceRows[i];const photo=row.querySelector('input[type="file"]');if(!photo?.files?.[0]&&!photo?.dataset.existing){goListingStep(4);openModal('Experience photo required','Har bookable experience ke liye actual experience ka photo add karna zaroori hai.');return;}}
 if(f.elements.stayAvailable?.value==='Yes'){const stay=f.elements.stayPhoto;if(!stay?.files?.[0]&&!getSavedListing()?.stayPhoto&&!getSavedListing()?.media?.stay){goListingStep(10);openModal('Stay photo required','Stay available hai, isliye actual stay / homestay ka photo add karna zaroori hai.');return;}}
...
async function feedLike(id){if(!(await requireGoogleAuth()))return;const key=`villagedeko_feed_likes_${firebaseUser.uid}`,liked=vdUserArray(key),has=liked.includes(id);vdSetUserArray(key,has?liked.filter(x=>x!==id):[...liked,id]);renderVillageFeed();}
...
async function feedSave(id){if(!(await requireGoogleAuth()))return;const p=findFeedPost(id);if(!p)return;const key='villagedeko_saved_feed_posts',saved=vdUserArray(key),has=saved.some(x=>x.id===id);vdSetUserArray(key,has?saved.filter(x=>x.id!==id):[...saved,{...p,savedAt:Date.now()}]);renderVillageFeed();}
