...
onAuthStateChanged(auth, async (user) => {
  ...
  const displayName = user.displayName || email.split("@")[0] || "VillageDeko User";

  $("profileUserName").textContent = displayName;
  $("profileUserEmail").textContent = email || "Phone account";
  $("profileUserUid").textContent = "UID: " + user.uid;
