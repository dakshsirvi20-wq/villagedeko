...
async function initFirebase(){
 try{
  ...
  onAuthStateChanged(firebaseAuth, async user=>{ firebaseUser=user||null; if(!firebaseUser){localStorage.removeItem('villagedeko_user_role');} updateAuthUI(); applyUserRoleUI(); await loadRemoteListing(); await loadMarketplaceProducts(); });
...
function openRoleMenu(){
 ...
 ${isHost?'<button class="secondary" data-action="listing">🏡 My Village Listing</button><button class="secondary" data-action="bazaar-form">🛒 Add Bazaar Product</button><button class="secondary" data-action="switch-user">Switch to User</button>':'<button class="secondary" data-action="switch-host">Become a Host</button>'}
...
async function uploadCloudinary(file,fingerprint=''){
 ...
 const mediaKind=file.type.startsWith('video/')||file.type.startsWith('audio/')?'video':'image';
 const res=await fetch(`https://api.cloudinary.com/v1_1/${cfg.CLOUDINARY_CLOUD_NAME}/${mediaKind}/upload`,{method:'POST',body:fd});
...
function stayFileInputExisting(form){
 const el=form?.elements?.stayPhoto;
 return el?.dataset?.existing||'';
}
...
async function persistVillageToFirebase(data){
 ...
 // Upload row-level photo, video and voice media while keeping each item isolated.
 // Each uploaded media URL stays inside the same row, so one village never borrows another village's media.
 for(const type of ['explore','experience','day','food','journey','people','culture','package']){
  if(!Array.isArray(data[type]))continue;
  const cards=[...document.querySelectorAll(`#${type}-repeat .repeat-card`)];
  const defs=listingTemplates[type]||[];
  for(let i=0;i<data[type].length;i++){
   const fields=[...cards[i]?.querySelectorAll('input,select,textarea')||[]];
   for(let j=0;j<defs.length;j++){
    if(!['image','video','audio'].includes(defs[j]?.[1]))continue;
    const input=fields[j];
    const file=input?.files?.[0];
    if(file){
     const uploaded=await reuseOrUploadCloudinary(file,[]);
     if(uploaded?.url)data[type][i][j]=uploaded.url;
    }else if(input?.dataset.existing){
     data[type][i][j]=input.dataset.existing;
    }
   }
  }
 }

 const stayFile=f?.elements.stayPhoto?.files?.[0];
 if(stayFile){
  const uploadedStay=await reuseOrUploadCloudinary(stayFile,existingMedia.stay?[existingMedia.stay]:[]);
  if(uploadedStay?.url)data.stayPhoto=uploadedStay;
 }else if(stayFileInputExisting(f)){
  data.stayPhoto=stayFileInputExisting(f);
 }else if(existingMedia.stay){
  data.stayPhoto=existingMedia.stay;
 }
 const stayVideoInput=f?.elements?.stayVideo;
 const stayVideoFile=stayVideoInput?.files?.[0];
 if(stayVideoFile){
  const uploadedStayVideo=await reuseOrUploadCloudinary(stayVideoFile,existingMedia.stayVideo?[existingMedia.stayVideo]:[]);
  if(uploadedStayVideo?.url)data.stayVideo=uploadedStayVideo;
 }else if(stayVideoInput?.dataset?.existing){
  data.stayVideo=stayVideoInput.dataset.existing;
 }else if(existingMedia.stayVideo){
  data.stayVideo=existingMedia.stayVideo;
 }
 ...
 data.media={
  ...
  photos:mergedPhotos,
  videos:mergedVideos,
  stay:data.stayPhoto||existingMedia.stay||null,
  stayVideo:data.stayVideo||existingMedia.stayVideo||null
 };
...
initFirebase();
const ALL_INDIA_IMAGE='assets/states/india-gate.jpg';
const STATE_CULTURE_IMAGES={
 'Andhra Pradesh':'assets/states/andhra-pradesh.jpg','Arunachal Pradesh':'assets/states/arunachal-pradesh.jpg','Assam':'assets/states/assam.jpg','Bihar':'assets/states/bihar.jpg','Chhattisgarh':'assets/states/chhattisgarh.jpg','Goa':'assets/states/goa.jpg','Gujarat':'assets/states/gujarat.jpg','Haryana':'assets/states/haryana.jpg','Himachal Pradesh':'assets/states/himachal-pradesh.jpg','Jharkhand':'assets/states/jharkhand.jpg','Karnataka':'assets/states/karnataka.jpg','Kerala':'assets/states/kerala.jpg','Madhya Pradesh':'assets/states/madhya-pradesh.jpg','Maharashtra':'assets/states/maharashtra.jpg','Manipur':'assets/states/manipur.jpg','Meghalaya':'assets/states/meghalaya.jpg','Mizoram':'assets/states/mizoram.jpg','Nagaland':'assets/states/nagaland.jpg','Odisha':'assets/states/odisha.jpg','Punjab':'assets/states/punjab.jpg','Rajasthan':'assets/states/rajasthan.jpg','Sikkim':'assets/states/sikkim.jpg','Tamil Nadu':'assets/states/tamil-nadu.jpg','Telangana':'assets/states/telangana.jpg','Tripura':'assets/states/tripura.jpg','Uttar Pradesh':'assets/states/uttar-pradesh.jpg','Uttarakhand':'assets/states/uttarakhand.jpg','West Bengal':'assets/states/west-bengal.jpg','Andaman and Nicobar Islands':'assets/states/andaman-and-nicobar-islands.jpg','Chandigarh':'assets/states/chandigarh.jpg','Dadra and Nagar Haveli and Daman and Diu':'assets/states/dadra-and-nagar-haveli-and-daman-and-diu.jpg','Delhi':'assets/states/delhi.jpg','Jammu and Kashmir':'assets/states/jammu-and-kashmir.jpg','Ladakh':'assets/states/ladakh.jpg','Lakshadweep':'assets/states/lakshadweep.webp','Puducherry':'assets/states/puducherry.jpg'
};
function stateCultureImage(state){return state==='all'?ALL_INDIA_IMAGE:(STATE_CULTURE_IMAGES[state]||'');}
// Demo content uses the supplied state culture image only as a neutral placeholder.
// Real village/profile media comes from each host's own uploads.
function contentImage(state){return stateCultureImage(state)||ALL_INDIA_IMAGE;}
const villages=[
{id:'kuthar',name:'Kuthar Village',district:'Jodhpur District',state:'Rajasthan',cover:stateCultureImage('Rajasthan'),about:'A living desert village experience with traditional homes, farming, craft, food and warm local hospitality.',best:'Oct – March',reach:'Jodhpur Airport · 85 km',language:'Marwari · Hindi'},
{id:'majuli',name:'Majuli Village',district:'Majuli District',state:'Assam',cover:stateCultureImage('Assam'),about:'Island village life, river landscapes, mask making, local food and living traditions.',best:'Nov – April',reach:'Jorhat · Ferry',language:'Assamese · Mising'},
{id:'hampi',name:'Hampi Village',district:'Vijayanagara',state:'Karnataka',cover:stateCultureImage('Karnataka'),about:'Village life beside historic landscapes, local farms, food, crafts and heritage.',best:'Oct – February',reach:'Hospet · 15 km',language:'Kannada · Hindi'},
{id:'bhuj',name:'Bhujodi Village',district:'Kutch District',state:'Gujarat',cover:stateCultureImage('Gujarat'),about:'Textiles, crafts, food and artisan traditions of rural Kutch.',best:'Nov – February',reach:'Bhuj · 10 km',language:'Gujarati · Hindi'},
{id:'punjab',name:'Rural Punjab Village',district:'Amritsar District',state:'Punjab',cover:stateCultureImage('Punjab'),about:'Fields, dairy, Punjabi food, folk music and everyday village hospitality.',best:'Oct – March',reach:'Amritsar · 30 km',language:'Punjabi · Hindi'}];
const stories=[
{villageId:'kuthar',title:'40 years of farming, one field at a time.',tag:'Village Life',category:'village-life',person:'Gopal Singh · Farmer',text:'Gopal shares how one family field changed through four decades — from seed and water to harvest and mandi.',img:contentImage('Rajasthan','farmer field')},
{villageId:'kuthar',title:'The recipe our village remembers.',tag:'Local Food Story',category:'local-food',person:'Sita Devi · Local Cook',text:'A traditional meal, cooked slowly and explained by the person who has carried the recipe through generations.',img:contentImage('Rajasthan','traditional food cooking')},
{villageId:'kuthar',title:'Clay, hands and a living craft.',tag:'People & Culture',category:'people-culture',person:'Hukamram · Potter',text:'Meet a village artisan and understand how local clay becomes everyday objects and keeps an old skill alive.',img:contentImage('Rajasthan','pottery artisan')},
{villageId:'kuthar',title:'The traditions that bring the village together.',tag:'Festivals / Traditions',category:'festivals-traditions',person:'Village Elders · Community Story',text:'A look at the shared customs, music, food and gatherings that keep village traditions alive across generations.',img:contentImage('Rajasthan','folk festival')},
{villageId:'punjab',title:'My field, my life, my story.',tag:'Gaon Ke Log',category:'gaon-ke-log',person:'Ramesh ji · Farmer',text:'A farmer explains what a normal day in the field looks like, why each season matters and what farming means to his family.',img:contentImage('Punjab','farmer field')},
{villageId:'punjab',title:'The animals are part of our family.',tag:'Gaon Ke Log',category:'gaon-ke-log',person:'Kamla Devi · Pashupalak',text:'A village livestock keeper shares the daily rhythm of feeding, caring for animals and the role dairy plays in village life.',img:contentImage('Punjab','dairy village')},
{villageId:'bhuj',title:'Keeping an old craft alive.',tag:'Gaon Ke Log',category:'gaon-ke-log',person:'Mohan ji · Local Artisan',text:'A local artisan shares how a traditional skill is learned, practiced and passed from one generation to the next.',img:contentImage('Gujarat','artisan handicraft')}
];
const experiences=[
['activities','Farm Visit','2 Hours · Sow, harvest, feed animals and walk the fields','700',contentImage('Rajasthan','farm experience')],
['activities','Pottery Workshop','1.5 Hours · Shape clay with a local village artisan','800',contentImage('Rajasthan','pottery workshop')],
['food','Traditional Thali','2 Hours · Help prepare and eat a traditional local meal','450',contentImage('Rajasthan','traditional thali')],
['culture','Folk Evening','2 Hours · Music, dance, stories and local traditions','500',contentImage('Rajasthan','folk evening')]
];
const villageExperiences={
'kuthar':experiences,
'majuli':[['activities','Island Farming','2 Hours · Walk the fields and learn local farming','650',contentImage('Assam','island farming')],['food','Assamese Cooking','2 Hours · Cook a traditional village meal with a host','500',contentImage('Assam','assamese cooking')],['culture','Folk Culture Evening','2 Hours · Music, dance and village stories','550',contentImage('Assam','folk culture')]],
'kuttanad':[['activities','Backwater Village Walk','2 Hours · Walk village paths and meet local families','700',contentImage('Kerala','backwater village')],['food','Kerala Home Cooking','2 Hours · Prepare a traditional local lunch','600',contentImage('Kerala','home cooking')],['culture','Village Life Evening','2 Hours · Local stories, music and traditions','500',contentImage('Kerala','village culture')]]
};
function getVillageExperiences(v=currentVillage){
 const listing=Array.isArray(v?.listing?.experience)?v.listing.experience.filter(x=>Array.isArray(x)&&x[0]):[];
 if(listing.length)return listing.map(x=>[String(x[1]||'Activities').toLowerCase(),x[0]||'Village Experience',[x[2],x[4]].filter(Boolean).join(' · '),x[3]||0,x[5]||'',x[6]||'']);
 return v?.id&&villageExperiences[v.id]?villageExperiences[v.id]:[];
}
function getGlobalExperiences(){
 const out=[];
 villages.forEach(v=>{getVillageExperiences(v).forEach(e=>out.push({village:v,experience:e}));});
 return out;
}
...
function getTrips(){
 if(currentVillage){
  const rows=currentVillage.listing?.package||[];
  const custom=rows.filter(r=>Array.isArray(r)&&r[0]).map(r=>({name:r[0]||`${currentVillage.name} Village Package`,duration:r[1]||'1 Day',culture:r[2]||'Complete village experience',included:(r[3]||'').split(',').map(x=>x.trim()).filter(Boolean),stay:r[4]||currentVillage.listing?.stayDetails||'Stay as listed',food:r[5]||'Meals as listed',host:r[6]||currentVillage.ownerName||'Village Host',price:Number(r[7]||0),childPrice:Number(r[8]||0),minGuests:Number(r[9]||1),maxGuests:Number(r[10]||20),availability:r[11]||`Available ${currentVillage.best||'seasonal'}`,notIncluded:(r[12]||'Transport to/from village, Personal expenses').split(',').map(x=>x.trim()).filter(Boolean),photo:r[13]||'',video:r[14]||''}));
  if(custom.length)return custom;
  return tripPackages[currentVillage.id]||[{name:`${currentVillage.name} Village Escape`,duration:'1 Day',stay:'Local village homestay',food:'Breakfast + traditional local meal',culture:'Local village life, stories and activities',included:getVillageExperiences().slice(0,3).map(e=>e[1]),notIncluded:['Transport to the village','Personal expenses','Insurance'],host:'Local Village Host',availability:`Available ${currentVillage.best||'seasonal'}`,price:2500}];
 }
 return villages.flatMap(v=>{const list=tripPackages[v.id]||[{name:`${v.name} Village Escape`,duration:'1 Day',stay:'Local village homestay',food:'Breakfast + traditional local meal',culture:'Local village life, stories and activities',included:(villageExperiences[v.id]||experiences).slice(0,3).map(e=>e[1]),notIncluded:['Transport to the village','Personal expenses','Insurance'],host:'Local Village Host',availability:`Available ${v.best||'seasonal'}`,price:2500}];return list.map(trip=>({trip,village:v}));});
}
function renderTrips(){
 const title=document.getElementById('trip-village-title'),grid=document.getElementById('trip-grid');if(!grid)return;
 const currentBtn=document.querySelector('[data-action="trip-current"]');
 if(currentVillage){
  if(title)title.textContent=`Trips in ${currentVillage.name}`;
  if(currentBtn)currentBtn.textContent='Current village →';
  grid.innerHTML=getTrips().map((t,i)=>{const v=currentVillage;return `<article class="trip-card"><div class="trip-image"><img src="${esc(v.cover)}" alt="${esc(t.name)}"><span>${esc(t.duration)}</span></div><div class="trip-body"><span class="trip-label">COMPLETE VILLAGE PACKAGE</span><h3>${esc(t.name)}</h3><p class="trip-location">${esc(v.name)} · ${esc(v.district)} · ${esc(v.state)}</p><div class="trip-facts"><span><b>Stay</b>${esc(t.stay)}</span><span><b>Food</b>${esc(t.food)}</span><span><b>Host</b>${esc(t.host)}</span><span><b>Availability</b>${esc(t.availability)}</span></div><div class="trip-block"><b>Experiences / Activities</b><div class="trip-chips">${t.included.slice(0,4).map(x=>`<span>${esc(x)}</span>`).join('')}</div></div><div class="trip-columns"><div><b>What's Included</b><ul>${t.included.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div><div><b>What's Not Included</b><ul>${t.notIncluded.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div></div><div class="trip-footer"><div><small>Total Trip Price</small><strong>₹${Number(t.price||0).toLocaleString('en-IN')}</strong></div><button class="primary" data-action="book-trip" data-trip="${i}">Book Trip</button></div></div></article>`;}).join('');
  return;
 }
 if(title)title.textContent='Trips across India';if(currentBtn)currentBtn.textContent='Choose village →';
 const entries=getTrips();
 grid.innerHTML=entries.map(({trip:t,village:v},i)=>`<article class="trip-card"><div class="trip-image"><img src="${esc(v.cover)}" alt="${esc(t.name)}"><span>${esc(t.duration)}</span></div><div class="trip-body"><span class="trip-label">${esc(v.name.toUpperCase())}</span><h3>${esc(t.name)}</h3><p class="trip-location">${esc(v.name)} · ${esc(v.district)} · ${esc(v.state)}</p><div class="trip-facts"><span><b>Stay</b>${esc(t.stay)}</span><span><b>Food</b>${esc(t.food)}</span><span><b>Host</b>${esc(t.host)}</span><span><b>Availability</b>${esc(t.availability)}</span></div><div class="trip-block"><b>Experiences / Activities</b><div class="trip-chips">${t.included.slice(0,4).map(x=>`<span>${esc(x)}</span>`).join('')}</div></div><div class="trip-columns"><div><b>What's Included</b><ul>${t.included.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div><div><b>What's Not Included</b><ul>${t.notIncluded.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div></div><div class="trip-footer"><div><small>Total Trip Price</small><strong>₹${Number(t.price||0).toLocaleString('en-IN')}</strong></div><button class="primary" data-action="book-trip" data-trip="${i}">Book Trip</button></div></div></article>`).join('');
}
const products=[
 {name:'Bajra & Village Grains',price:320,image:contentImage('Rajasthan','bajra grains'),category:'Food & Grains',seller:'Gopal Singh · Farmer',village:'Kuthar Village · Jodhpur District · Rajasthan',story:'40 years of farming, one field at a time — a farmer story about seed, water, harvest and the life behind village grains.'},
 {name:'Raw Village Honey',price:550,image:contentImage('Rajasthan','village honey'),category:'Honey',seller:'Kuthar Village Farmer Collective',village:'Kuthar Village · Jodhpur District · Rajasthan',story:'Honey from village-side farming and local work, connected to the people and landscape that produce it.'},
 {name:'Hand-ground Red Chilli',price:280,image:contentImage('Rajasthan','red chilli spice'),category:'Spices',seller:'Kuthar Village Women Producers',village:'Kuthar Village · Jodhpur District · Rajasthan',story:'A local spice story that connects everyday cooking with the ingredients grown, prepared and used in the village.'},
 {name:'Handmade Pottery',price:650,image:contentImage('Rajasthan','handmade pottery'),category:'Handicrafts',seller:'Hukamram · Potter',village:'Kuthar Village · Jodhpur District · Rajasthan',story:'Clay, hands and a living craft — understand how local clay becomes everyday objects and keeps an old skill alive.'},
 {name:'Hand-painted Folk Art',price:900,image:contentImage('Gujarat','folk art'),category:'Local Art',seller:'Mohan ji · Local Artisan',village:'Bhujodi Village · Kutch District · Gujarat',story:'Keeping an old craft alive — a local artisan story about learning, practicing and passing a traditional skill forward.'},
 {name:'Handwoven Village Textile',price:750,image:contentImage('Gujarat','handwoven textile'),category:'Textiles',seller:'Bhujodi Artisan Collective',village:'Bhujodi Village · Kutch District · Gujarat',story:'Village-made textile work that carries local craft knowledge from the artisan’s hands into everyday life.'},
 {name:'Handcrafted Utility Basket',price:450,image:contentImage('Rajasthan','handmade basket'),category:'Handmade Products',seller:'Kuthar Craft Collective',village:'Kuthar Village · Jodhpur District · Rajasthan',story:'A useful handmade object with a village story — made through local skill, time and generations of craft knowledge.'}
];

let marketplaceProducts=[...products];
function getSavedBazaarProducts(){try{return JSON.parse(localStorage.getItem('villagedeko_my_bazaar_products')||'[]')||[];}catch(_e){return [];}}
function setSavedBazaarProducts(items){localStorage.setItem('villagedeko_my_bazaar_products',JSON.stringify(items||[]));}
function mergeMarketplaceProducts(items){
 const map=new Map();
 [...products,...getSavedBazaarProducts(),...(Array.isArray(items)?items:[])].forEach(p=>{if(p?.id)map.set(p.id,p);else map.set(`static-${map.size}`,p);});
 marketplaceProducts=[...map.values()];
}
async function loadMarketplaceProducts(){
 mergeMarketplaceProducts();
 if(!firestoreDb||!window.VD_FIREBASE)return;
 try{
  const snap=await window.VD_FIREBASE.getDocs(window.VD_FIREBASE.collection(firestoreDb,'marketplaceProducts'));
  const remote=[];snap.forEach(doc=>remote.push({id:doc.id,...(doc.data()||{})}));
  mergeMarketplaceProducts(remote);renderMarket();
 }catch(e){console.warn('Marketplace products could not be loaded from Firebase; local products remain visible.',e);}
}
function populateBazaarVillageOptions(){
 const s=document.getElementById('bazaar-village');if(!s)return;
 const previous=s.value;
 const list=dedupeVillagesByIdentity(villages);
 s.innerHTML='<option value="">Select village</option>'+list.map(v=>`<option value="${esc(v.id)}">${esc(v.name)} · ${esc(v.district)} · ${esc(v.state)}</option>`).join('');
 if(previous&&list.some(v=>v.id===previous))s.value=previous;
 if(currentVillage&&list.some(v=>v.id===currentVillage.id)&&!s.value)s.value=currentVillage.id;
 updateBazaarVillageFields();
}
function updateBazaarVillageFields(){
 const s=document.getElementById('bazaar-village');if(!s)return;
 const v=villages.find(x=>x.id===s.value);const d=document.getElementById('bazaar-district');const st=document.getElementById('bazaar-state');
 if(d)d.value=v?.district||'';if(st)st.value=v?.state||'';
}
function getBazaarFormData(){
 const f=document.getElementById('bazaar-product-form');if(!f)return null;
 const fd=new FormData(f);const v=villages.find(x=>x.id===fd.get('villageId'));
 return {productName:String(fd.get('productName')||'').trim(),category:String(fd.get('category')||''),price:Number(fd.get('price')||0),stock:Number(fd.get('stock')||0),description:String(fd.get('description')||'').trim(),sellerName:String(fd.get('sellerName')||'').trim(),sellerType:String(fd.get('sellerType')||''),sellerStory:String(fd.get('sellerStory')||'').trim(),villageId:v?.id||'',villageName:v?.name||'',district:v?.district||'',state:v?.state||'',story:String(fd.get('story')||'').trim(),buyNote:String(fd.get('buyNote')||'').trim(),sellerContact:String(fd.get('sellerContact')||'').trim()};
}
function renderBazaarProductReview(){
 const root=document.getElementById('bazaar-product-review');if(!root)return;const d=getBazaarFormData()||{};
 root.innerHTML=[['Product',d.productName||'Not added',`${d.category||''} · ₹${Number(d.price||0).toLocaleString('en-IN')}`],['Seller',d.sellerName||'Not added',d.sellerType||''],['Village',d.villageName||'Not added',`${d.district||''} · ${d.state||''}`],['Story',d.story||'Not added',''],['Buy',d.buyNote||'Not added',d.stock?`${d.stock} available`:'Stock not specified']].map(x=>`<div class="review-card"><small>${esc(x[0])}</small><strong>${esc(x[1])}</strong><p>${esc(x[2])}</p></div>`).join('');
}
function initBazaarProductForm(){
 const f=document.getElementById('bazaar-product-form');if(!f||f.dataset.initialized)return;
 f.dataset.initialized='1';populateBazaarVillageOptions();
 f.addEventListener('input',renderBazaarProductReview);f.addEventListener('change',e=>{if(e.target.id==='bazaar-village')updateBazaarVillageFields();renderBazaarProductReview();});renderBazaarProductReview();
}
function openBazaarProductForm(){
 if(getUserRole()!=='host'){openRoleChooser();return;}
 populateBazaarVillageOptions();initBazaarProductForm();document.getElementById('bazaar-product-form')?.reset();populateBazaarVillageOptions();renderBazaarProductReview();showSlide('slide-bazaar-form');closeModal();
}
async function saveBazaarProduct(){
 const f=document.getElementById('bazaar-product-form');if(!f)return;if(!f.checkValidity()){f.reportValidity();return;}
 if(!(await requireGoogleAuth()))return;
 const d=getBazaarFormData();if(!d.villageId){openModal('Village required','Please connect the product to its real village before publishing it.');return;}
 try{
  const file=f.elements.productImage?.files?.[0];let image=contentImage('Rajasthan','village product');
  if(file){const uploaded=await reuseOrUploadCloudinary(file,[]);if(uploaded?.url)image=uploaded.url;}
  const id=`${firebaseUser.uid}_${Date.now()}`;
  const product={id,ownerUid:firebaseUser.uid,ownerName:firebaseUser.displayName||'',...d,image,createdAt:Date.now(),updatedAt:Date.now()};
  const saved=[...getSavedBazaarProducts().filter(x=>x.id!==id),product];setSavedBazaarProducts(saved);
  if(firestoreDb)await window.VD_FIREBASE.setDoc(window.VD_FIREBASE.doc(firestoreDb,'marketplaceProducts',id),product,{merge:true});
  mergeMarketplaceProducts([product]);renderMarket();openModal('Bazaar Product Saved',`${esc(product.productName)} is now connected to ${esc(product.villageName)} and its story. It can appear in the global Gaon Ka Bazaar.`);
 }catch(e){console.error('Bazaar product save failed',e);openModal('Could not save Bazaar product',e?.message||'Please try again.');}
}
const states=['Andhra Pradesh','Arunachal Pradesh','Assam','Bihar','Chhattisgarh','Goa','Gujarat','Haryana','Himachal Pradesh','Jharkhand','Karnataka','Kerala','Madhya Pradesh','Maharashtra','Manipur','Meghalaya','Mizoram','Nagaland','Odisha','Punjab','Rajasthan','Sikkim','Tamil Nadu','Telangana','Tripura','Uttar Pradesh','Uttarakhand','West Bengal','Andaman and Nicobar Islands','Chandigarh','Dadra and Nagar Haveli and Daman and Diu','Delhi','Jammu and Kashmir','Ladakh','Lakshadweep','Puducherry'];
let currentVillage=null, selectedState='all';
function setCurrentVillage(v){currentVillage=v||null;if(currentVillage)localStorage.setItem('villagedeko_selected_village',currentVillage.id||'');else localStorage.removeItem('villagedeko_selected_village');}
function restoreSelectedVillage(){const id=localStorage.getItem('villagedeko_selected_village')||'';if(id)currentVillage=villages.find(v=>v.id===id)||null;}
restoreSelectedVillage();
...
const navMap={
  ...
  'slide-explore':'slide-explore',
  'slide-profile':'slide-explore',
  'slide-market':'slide-explore',
  'slide-bazaar-form':'slide-explore',
  'slide-listing':'slide-explore',
  'slide-experiences':'slide-experiences',
  'slide-stay':'slide-stay',
  'slide-stories':'slide-stories'
};
const activeNav=navMap[id]||'';
document.querySelectorAll('.nav-item,.desktop-nav-item').forEach(n=>{
  const active=n.dataset.nav===activeNav;
...
function renderStates(){document.getElementById('state-slider').innerHTML=`<button class="state-card active state-card-all" data-state="all"><span class="state-card-shade"></span><img src="${ALL_INDIA_IMAGE}" alt="India Gate · All India"><span class="state-card-copy"><small>ALL INDIA</small><strong>All India</strong><em>${states.length} regions</em></span></button>`+states.map(s=>`<button class="state-card" data-state="${s}"><span class="state-card-shade"></span><img src="${stateCultureImage(s)}" alt="${esc(s)} culture"><span class="state-card-copy"><strong>${esc(s)}</strong></span></button>`).join('')}
function getVillageFilters(){return {search:(document.getElementById('village-search')?.value||'').trim().toLowerCase()}}
function renderVillages(state=selectedState||'all'){if(firebaseUser)syncSavedListingIntoVillages();const unique=dedupeVillagesByIdentity(villages);if(unique.length!==villages.length)villages.splice(0,villages.length,...unique);state=typeof state==='string'&&(['all',...states].includes(state))?state:'all';selectedState=state;const {search}=getVillageFilters();let list=state==='all'?villages:villages.filter(v=>v.state===state);if(search)list=list.filter(v=>`${v.name} ${v.district} ${v.state}`.toLowerCase().includes(search));document.getElementById('selected-state-label').textContent=state==='all'?'All India':state;const empty=document.getElementById('empty-state');empty.hidden=list.length>0;empty.innerHTML=list.length?empty.innerHTML:`<strong>No villages found.</strong><p>Try another village name or district, choose another state, or clear the filters.</p>`;document.getElementById('village-grid').innerHTML=list.map(v=>`<article class="village-card" data-action="village" data-id="${v.id}" role="button" tabindex="0" aria-label="Open ${esc(v.name)} village profile"><img src="${v.cover}" alt="${esc(v.name)}"><span class="village-card-shade"></span><div class="village-card-overlay"><h3>${esc(v.name)}</h3><p>${esc(v.district)}${v.district&&v.state?', ':''}${esc(v.state)}</p></div></article>`).join('');document.querySelectorAll('.state-card').forEach(x=>x.classList.toggle('active',x.dataset.state===state));document.getElementById('village-grid').scrollLeft=0}
function clearVillageFilters(){const q=document.getElementById('village-search');if(q)q.value='';renderVillages('all')}
...
function listingToVillage(d){
 ...
 const coverUrl=typeof coverMedia==='string' ? coverMedia : (coverMedia?.url||stateCultureImage(d.state||'all'));
 const listing={...d};
 listing.stayPhoto=d.stayPhoto||d.media?.stay||null;
 listing.experience=Array.isArray(d.experience)?d.experience.map(row=>{
  if(!Array.isArray(row))return row;
  return [row[1]||'activities',row[0]||'Experience',row[2]||'',row[3]||'0',row[5]||stateCultureImage(d.state||'all'),row[4]||''];
 }):[];
 return {
  ...
  best:d.bestTime||'Not listed',reach:reach||'Visit information not listed',language:d.languages||'Not listed',pin:d.pin||'',
  listing,fromListing:true
 };
}
...
function profile6(v){return `<div class="profile-backbar"><button class="back-btn" data-action="back-to-explore">← Back to Explore</button></div><div class="profile-banner"><img src="${esc(v.cover)}" alt="${esc(v.name)}"><div class="profile-overlay"><span class="eyebrow">VILLAGE HUB</span><h2>${esc(v.name)}</h2><p>${esc(v.district)} · ${esc(v.state)}${(v.pin||v.listing?.pin)?` · ${esc(v.pin||v.listing?.pin)}`:''}</p><div class="profile-action-row"><button class="primary profile-save-village" data-action="save-village">❤️ Save Village</button><button class="primary" data-profile-visit="1">🧳 Visit This Village</button><button class="secondary" data-action="enquire-village">📩 Enquire</button></div></div></div><div class="profile-body"><div><h3>About ${esc(v.name)}</h3><p>${esc(v.about)}</p></div><div class="facts"><span>☀ Best time<br><b>${esc(v.best)}</b></span><span>⌖ How to reach<br><b>${esc(v.reach)}</b></span><span>◉ Languages<br><b>${esc(v.language)}</b></span></div></div><div class="profile-tabs" id="profile-tabs"><button class="profile-tab active" data-p="1">Overview</button><button class="profile-tab" data-p="2">Explore</button><button class="profile-tab" data-p="3">Experiences</button><button class="profile-tab" data-p="4">Ek Din Gaon Mein</button><button class="profile-tab" data-p="5">Food</button><button class="profile-tab" data-p="6">People & Stories</button><button class="profile-tab" data-p="7">Visit</button></div><div id="profile-pages">${profilePage(1,v)}</div>`}
...
function ensureProfileMediaStyles(){
 ...
 .profile-stay-photo{width:100%;aspect-ratio:16/9;overflow:hidden;border-radius:18px;margin:14px 0;background:#eee}.profile-stay-photo img{width:100%;height:100%;object-fit:cover;display:block}
 .existing-file-note{display:block;margin-top:6px}
...
function rowMediaMarkup(row,photoIndex,videoIndex,alt='Village media',audioIndex=-1){const photo=row?.[photoIndex]||'',video=row?.[videoIndex]||'',audio=audioIndex>=0?(row?.[audioIndex]||''):'';if(!photo&&!video&&!audio)return '';return `<div class="hub-row-media">${photo?`<img src="${esc(photo)}" alt="${esc(alt)}">`:''}${video?`<video src="${esc(video)}" controls playsinline preload="metadata"></video>`:''}${audio?`<audio src="${esc(audio)}" controls preload="metadata" aria-label="${esc(alt)} local voice"></audio>`:''}</div>`;}
function profilePage(n,v){
 const common=`<div class="profile-page">`, d=v.listing||null;
 if(!d&&n===4)return common+`<span class="eyebrow">EK DIN GAON MEIN</span><h3>Gaon ka ek poora din</h3><div class="hub-day-timeline">${ekDinGaonMein.map(x=>`<article class="hub-day-slot"><time>${x[0]}</time><div><strong>${x[3]} ${x[1]}</strong><p>${x[2]}</p></div></article>`).join('')}</div></div>`;
 if(!d)return legacyProfilePage(n>4?n-1:n,v,common);
 if(n===1){
  const media=d.media||{},cover=typeof media.cover==='string'?media.cover:(media.cover?.url||''),photos=Array.isArray(media.photos)?media.photos:[],videos=Array.isArray(media.videos)?media.videos:[];
  const photoItems=photos.map((m,i)=>{const url=typeof m==='string'?m:(m?.url||'');return url?`<figure class="profile-media-item" data-media-index="${cover?i+1:i}"><img src="${esc(url)}" alt="${esc(v.name)} photo ${i+1}" loading="lazy"></figure>`:''}).join('');
  const videoItems=videos.map(m=>{const url=typeof m==='string'?m:(m?.url||'');return url?`<figure class="profile-media-item"><video controls playsinline preload="metadata"><source src="${esc(url)}"></video></figure>`:''}).join('');
  const mediaBlock=(cover||photoItems||videoItems)?`<section class="profile-media-section">${cover?`<div class="profile-cover-media" data-media-index="0"><img src="${esc(cover)}" alt="${esc(v.name)} cover photo" loading="lazy"></div>`:''}${photoItems?`<div class="profile-media-heading"><span>PHOTOS</span><strong>${photos.length}</strong></div><div class="profile-media-grid">${photoItems}</div>`:''}${videoItems?`<div class="profile-media-heading"><span>VIDEOS</span><strong>${videos.length}</strong></div><div class="profile-video-grid">${videoItems}</div>`:''}</section>`:'';
  const feedPosts=Array.isArray(v.feedPosts)?v.feedPosts.slice().sort((a,b)=>Number(b.createdAt||0)-Number(a.createdAt||0)):[];
  const feedBlock=feedPosts.length?`<section class="profile-village-feed"><div class="profile-media-heading"><span>REAL VILLAGE FEED</span><strong>${feedPosts.length}</strong></div>${feedPosts.map(p=>`<article class="profile-feed-card">${p.media?.url?(p.media?.resourceType==='video'?`<video controls playsinline preload="metadata"><source src="${esc(p.media.url)}"></video>`:`<img src="${esc(p.media.url)}" alt="${esc(p.title||'Village update')}" loading="lazy">`):''}<div><small>HOST UPDATE · ${esc(p.authorName||'Village Host')}</small><h4>${esc(p.title||'Village update')}</h4><p>${esc(p.text||'')}</p><span>📍 ${esc(v.name)} · ${esc(v.district)} · ${esc(v.state)}</span></div></article>`).join('')}</section>`:'';
  return common+`<span class="eyebrow">OVERVIEW</span><h3>${esc(v.name)}</h3><p>${esc(d.about||'Village overview')}</p><div class="mini-grid hub-identity-grid"><div class="profile-data-card"><small>VILLAGE</small><strong>${esc(d.villageName||v.name)}</strong></div><div class="profile-data-card"><small>DISTRICT</small><strong>${esc(d.district||v.district)}</strong></div><div class="profile-data-card"><small>STATE</small><strong>${esc(d.state||v.state)}</strong></div><div class="profile-data-card"><small>PIN CODE</small><strong>${esc(d.pin||'Not listed')}</strong></div><div class="profile-data-card"><small>BEST TIME</small><strong>${esc(d.bestTime||'Not listed')}</strong></div><div class="profile-data-card"><small>LANGUAGES</small><strong>${esc(d.languages||'Not listed')}</strong></div></div>${mediaBlock}${feedBlock}</div>`;
 }
 if(n===2){const rows=d.explore||[];return common+`<span class="eyebrow">EXPLORE</span><h3>Explore ${esc(v.name)}</h3><p>Places and village life belonging to this village.</p><div class="mini-grid village-explore-grid">${rows.length?rows.map((x,i)=>`<article class="profile-data-card clickable-data" data-action="listing-place" data-index="${i}">${rowMediaMarkup(x,3,4,x[0]||'Village place')}<span class="data-icon">📍</span><small>${esc(x[1]||'Village Place')}</small><h4>${esc(x[0]||'Place')}</h4><p>${esc(x[2]||'')}</p></article>`).join(''):`<div class="empty-profile-data"><strong>No Explore places added yet.</strong></div>`}</div></div>`}
 if(n===3){const rows=d.experience||[];return common+`<span class="eyebrow">EXPERIENCES</span><h3>Experiences in ${esc(v.name)}</h3><div class="experience-list">${rows.length?rows.map((x,i)=>`<article class="experience profile-form-experience">${rowMediaMarkup(x,5,6,x[0]||'Experience')}<div><span class="experience-kicker">${esc(x[1]||'EXPERIENCE')}</span><h3>${esc(x[0]||'Experience')}</h3><p>${esc(x[4]||'')}</p><small>${esc(x[2]||'')}</small></div><strong>₹${esc(x[3]||'0')} <small>/ person</small></strong><button class="primary" data-action="book-experience" data-village="${esc(v.id)}" data-experience="${esc(x[0]||'')}">Book</button></article>`).join(''):`<div class="empty-profile-data"><strong>No experiences added yet.</strong></div>`}</div></div>`}
 if(n===4){const rows=(d.day||[]).filter(x=>Array.isArray(x)&&x[0]);return common+`<span class="eyebrow">EK DIN GAON MEIN</span><h3>Gaon ka ek poora din</h3><p>Real village moments, local people aur unki awaaz ke saath.</p><div class="hub-day-timeline">${rows.length?rows.map(x=>`<article class="hub-day-slot hub-day-real"><time>${esc(x[0]||'')}</time>${rowMediaMarkup(x,3,4,x[1]||'Village activity',5)}<div><strong>${esc(x[1]||'Village activity')}</strong>${x[6]?`<small>👤 ${esc(x[6])} · 📍 ${esc(v.name)} · ${esc(v.district)} · ${esc(v.state)}</small>`:''}<p>${esc(x[2]||'')}</p></div></article>`).join(''):ekDinGaonMein.map(x=>`<article class="hub-day-slot"><time>${x[0]}</time><div><strong>${x[3]} ${x[1]}</strong><p>${x[2]}</p></div></article>`).join('')}</div></div>`}
 if(n===5){const foods=d.food||[],journey=d.journey||[];return common+`<span class="eyebrow">FOOD</span><h3>Local Food & Food Journey</h3><div class="food-dishes">${foods.length?foods.map(x=>`<article class="profile-data-card">${rowMediaMarkup(x,3,4,x[0]||'Local food')}<small>${esc(x[1]||'LOCAL FOOD')}</small><h4>🍲 ${esc(x[0]||'Dish')}</h4><p>${esc(x[2]||'')}</p></article>`).join(''):`<div class="empty-profile-data"><strong>No local dishes added yet.</strong></div>`}</div>${journey.length?`<div class="hub-journey-grid">${journey.map((x,i)=>`<article class="profile-data-card clickable-data" data-action="listing-journey" data-index="${i}">${rowMediaMarkup(x,4,5,x[0]||'Food journey stage',7)}<small>FOOD JOURNEY · ${esc(x[6]||'LOCAL VILLAGE')}</small><h4>${esc(x[0]||'Stage')}</h4><p>${esc(x[3]||'')}</p><span>📍 ${esc(v.name)} · ${esc(v.district)} · ${esc(v.state)}</span></article>`).join('')}</div>`:''}</div>`}
 if(n===6){const people=d.people||[],culture=d.culture||[];return common+`<span class="eyebrow">PEOPLE & STORIES</span><h3>People, Stories & Culture</h3><div class="story-mini-grid">${people.length?people.map((x,i)=>`<article class="profile-data-card clickable-data" data-action="listing-story" data-index="${i}">${rowMediaMarkup(x,4,5,x[0]||'Village person',6)}<span>${esc(x[1]||'Village Person')} · ${esc(v.name)} · ${esc(v.district)} · ${esc(v.state)}</span><h3>${esc(x[2]||x[0]||'Meri Kahani')}</h3><p><b>${esc(x[0]||'')}</b></p><p>${esc(x[3]||'')}</p></article>`).join(''):`<div class="empty-profile-data"><strong>No people or stories added yet.</strong></div>`}</div><div class="hub-culture-grid">${culture.length?culture.map(x=>`<article class="profile-data-card">${rowMediaMarkup(x,3,4,x[0]||'Village culture')}<small>${esc(x[1]||'RURAL CULTURE')}</small><h4>${esc(x[0]||'Culture')}</h4><p>${esc(x[2]||'')}</p></article>`).join(''):`<div class="empty-profile-data"><strong>No culture items added yet.</strong></div>`}</div></div>`}
 const packages=getTrips();
 const stayPhoto=typeof d.stayPhoto==='string'?d.stayPhoto:(d.stayPhoto?.url||'');
 const stayVideo=typeof d.stayVideo==='string'?d.stayVideo:(d.stayVideo?.url||'');
 return common+`<span class="eyebrow">VISIT</span><h3>Plan & Book ${esc(v.name)}</h3><div class="hub-reach-grid"><div class="profile-data-card"><small>✈ NEAREST AIRPORT</small><strong>${esc(d.airport||'Not listed')}</strong></div><div class="profile-data-card"><small>🚆 NEAREST RAILWAY</small><strong>${esc(d.railway||'Not listed')}</strong></div><div class="profile-data-card"><small>🛣 ROAD / BUS</small><strong>${esc(d.busRoad||'Not listed')}</strong></div><div class="profile-data-card"><small>🚕 LOCAL TRANSPORT</small><strong>${esc(d.localTransport||'Not listed')}</strong></div><div class="profile-data-card"><small>⏱ TRAVEL TIME</small><strong>${esc(d.travelTime||'Not listed')}</strong></div><div class="profile-data-card"><small>🌤 VISITOR TIPS</small><strong>${esc(d.weatherTips||'Not listed')}</strong></div></div><div class="hub-booking-choice"><article class="hub-book-card">${stayPhoto?`<img class="hub-stay-media" src="${esc(stayPhoto)}" alt="${esc(v.name)} stay">`:''}${stayVideo?`<video class="hub-stay-media" src="${esc(stayVideo)}" controls playsinline preload="metadata"></video>`:''}<small>STAY</small><h4>${esc(d.stayAvailable||'Not listed')}</h4><p>${esc(d.stayDetails||'Stay details not added.')}</p>${d.stayPrice?`<strong>₹${esc(d.stayPrice)} / night</strong><button class="primary" data-action="reserve">Book Stay / Visit</button>`:''}</article><article class="hub-book-card"><small>INDIVIDUAL EXPERIENCES</small><h4>Choose only what you want to do</h4><p>Farm, food, craft and other experiences listed by this village.</p><button class="secondary" data-profile-tab="3">See Experiences</button></article></div><div class="hub-package-head"><span class="eyebrow">COMPLETE VILLAGE PACKAGES</span><h3>Book the whole village experience</h3></div><div class="hub-package-grid">${packages.map((t,i)=>`<article class="hub-package-card">${t.photo?`<img class="hub-package-media" src="${esc(t.photo)}" alt="${esc(t.name)}">`:''}${t.video?`<video class="hub-package-media" src="${esc(t.video)}" controls playsinline preload="metadata"></video>`:''}<span>${esc(t.duration)}</span><h4>${esc(t.name)}</h4><p>${esc(t.culture||'Village life, stay, food and experiences')}</p><div class="hub-package-facts"><small>🏡 ${esc(t.stay||'Stay as listed')}</small><small>🍲 ${esc(t.food||'Meals as listed')}</small><small>📅 ${esc(t.availability||'Ask host')}</small></div><strong>₹${Number(t.price||0).toLocaleString('en-IN')} / person</strong><button class="primary" data-action="book-trip" data-trip="${i}">Book Complete Package</button></article>`).join('')}</div></div>`;
}
function legacyProfilePage(n,v,common){if(n===1)return common+`<span class="eyebrow"></span><h3>Welcome to ${esc(v.name)}</h3><p>Explore the village as a tourism experience — its landscape, homes, people, daily life and places worth visiting.</p><div class="mini-grid"><button class="secondary" data-action="photos">Village Photos</button><button class="secondary" data-action="videos">Village Videos</button><button class="primary" data-action="plan">Plan Your Visit</button></div></div>`;if(n===2)return common+`<span class="eyebrow"></span><h3>Explore the Village</h3><p>Discover this village section by section — places, daily life, food and culture.</p><div class="mini-grid village-explore-grid"><button class="secondary" data-action="place" data-place="Khet">🌾 Khet</button><button class="secondary" data-action="place" data-place="Dairy">🐄 Dairy</button><button class="secondary" data-action="place" data-place="Ghar & Daily Life">🏠 Ghar & Daily Life</button><button class="secondary" data-action="place" data-place="Mandir">🛕 Mandir</button><button class="secondary" data-action="place" data-place="Paani">💧 Paani</button><button class="secondary" data-action="place" data-place="School">🏫 School</button><button class="secondary" data-action="place" data-place="Local Market">🛍️ Local Market</button><button class="secondary" data-action="place" data-place="Local Food">🍲 Local Food</button><button class="secondary" data-action="place" data-place="Culture">🎵 Culture</button><button class="secondary" data-action="place" data-place="Local Art">🎨 Local Art</button></div></div>`;if(n===3)return common+`<span class="eyebrow"></span><h3>Experiences & Activities</h3><div class="experience-list">${getVillageExperiences().map(e=>`<article class="experience"><img src="${esc(e[4])}"><div><h3>${esc(e[1])}</h3><p>${esc(e[2])}</p></div><strong>₹${esc(e[3])}</strong><button data-action="book">Book</button></article>`).join('')}</div></div>`;if(n===4)return common+`<span class="eyebrow"></span><h3>Food Journey</h3><p>See how local food moves from village to your plate.</p><div class="journey">Seed <b>→</b> Field <b>→</b> Harvest <b>→</b> Mandi <b>→</b> Processing <b>→</b> Home <b>→</b> Plate</div><button class="primary" data-action="foodstory">Explore Local Food Stories</button></div>`;if(n===5)return common+`<span class="eyebrow"></span><h3>People, Stories & Culture</h3><div class="story-mini-grid">${stories.map((s,i)=>`<article><img src="${esc(s.img)}"><span>${esc(s.tag)}</span><h3>${esc(s.title)}</h3><p>${esc(s.person)}</p><button class="secondary" data-action="story" data-story="${i}">Read Story</button></article>`).join('')}</div><div class="culture-strip"><b>Rural Culture</b><span>Folk Music</span><span>Festivals</span><span>Marriage Traditions</span><span>Local Art</span></div></div>`;return common+`<span class="eyebrow"></span><h3>Stay & Visit</h3><div class="mini-grid"><button class="secondary" data-action="reserve">Village Stay</button><button class="secondary" data-action="plan">Plan Village Visit</button></div><p>Hosts can list stays and activities and set their own prices.</p></div>`}
function foodJourneyMarkup(){
 return `<article class="experience food-journey-experience-card" data-type="food-journey" data-action="food-journey-open" role="button" tabindex="0"><div class="food-journey-card-icon">🌾</div><div><span class="experience-kicker">FOOD JOURNEY · KNOWLEDGE</span><h3>Khaana gaon se hamari plate tak kaise pahunchta hai?</h3><p>Beej se lekar plate tak khaane ka poora safar step by step samjho.</p>${currentVillage?`<small>📍 ${esc(currentVillage.name)} · ${esc(currentVillage.district)} · ${esc(currentVillage.state)}</small>`:'<small>🌍 India · Knowledge Journey</small>'}</div><strong>9 stages</strong><button data-action="food-journey-open">Explore</button></article>`;
}
...
function renderFoodJourneyModal(){
 ...
 openModal('Food Journey — Knowledge', `<div class="food-journey-modal"><p class="modal-lead">Khaana sirf supermarket se shuru nahi hota. Uske peeche beej, khet, kisan aur poori journey hoti hai.</p>${currentVillage?`<p><b>📍 ${esc(currentVillage.name)} · ${esc(currentVillage.district)} · ${esc(currentVillage.state)}</b></p>`:'<p><b>🌍 India · Knowledge Journey</b></p>'}<div class="journey">${stages.map((x,i)=>`<button class="journey-stage" data-action="food-journey-stage-modal" data-stage="${i}">${x[0]} ${x[1]}</button>${i<stages.length-1?'<b>→</b>':''}`).join('')}</div><div class="food-journey-detail" id="food-journey-modal-detail"><h4>${stages[0][1]}</h4><p>${stages[0][2]}</p></div><p class="knowledge-note">Ye abhi knowledge experience hai. Real farmer, real village, photo/video/voice ko future ke Real Food Journey step mein connect kiya jayega.</p></div>`);
}
function renderExperiences(filter='all'){
 ...
 if(title)title.textContent=currentVillage?`Experiences in ${currentVillage.name}`:'Experiences across India';
 if(!root)return;
 if(filter==='ek-din'){renderEkDinGaonMein();return;}
 if(filter==='food-journey'){root.innerHTML=foodJourneyMarkup();return;}
 const source=currentVillage?getVillageExperiences().map(e=>({village:currentVillage,experience:e})):getGlobalExperiences();
 const filtered=filter==='all'?source:source.filter(({experience:e})=>e[0]===filter);
 const cards=filtered.map(({village:v,experience:e})=>`<article class="experience" data-type="${esc(e[0])}"><img src="${esc(e[4])}" alt="${esc(e[1])}"><div><span class="experience-kicker">${currentVillage?'VILLAGE EXPERIENCE':esc(v.name.toUpperCase())}</span><h3>${esc(e[1])}</h3><p>${esc(e[2])}</p><small>📍 ${esc(v.name)} · ${esc(v.district)} · ${esc(v.state)}</small></div><strong>₹${esc(e[3])} <small>/ person</small></strong><button data-action="book-experience" data-village="${esc(v.id)}" data-experience="${esc(e[1])}">Book</button></article>`).join('');
 const empty=!cards?`<div class="empty-state"><strong>${currentVillage?`No experiences listed in ${esc(currentVillage.name)} yet.`:'No experiences are listed across India yet.'}</strong><p>${currentVillage?'The village host can add experiences from the Village Listing form.':'Choose a village to explore its experiences, or return when more villages are listed.'}</p></div>`:'';
 ...
 root.innerHTML=ek+food+(cards||empty);
}

async function openExperienceBooking(villageId,experienceName){
 const v=villages.find(x=>x.id===villageId);
 if(!v)return;
 const list=getVillageExperiences(v);
 const e=list.find(x=>x[1]===experienceName);
 if(!e)return;
 setCurrentVillage(v);
 renderProfile();renderExperiences();renderTrips();renderStories();
 if(!(await requireGoogleAuth()))return;
 const today=new Date().toISOString().slice(0,10);
 openModal(`Book — ${e[1]}`,`<div class="vd-booking-header"><div class="vd-feature-card"><strong>${esc(e[1])}</strong><small>📍 ${esc(v.name)} · ${esc(v.district)} · ${esc(v.state)}</small></div><p class="vd-booking-note">Choose your date and guests. Your request will go to the village host for confirmation.</p></div><div class="vd-modal-form vd-booking-form"><div class="vd-form-field vd-form-field-full"><label>Experience</label><input value="${esc(e[1])}" readonly></div><div class="vd-form-field"><label>Visit date</label><input id="vd-experience-date" type="date" min="${today}"></div><div class="vd-form-field"><label>Guests</label><input id="vd-experience-guests" type="number" min="1" max="20" value="2"></div><div class="vd-form-field vd-form-field-full"><label>Message to host <span class="vd-optional">(optional)</span></label><textarea id="vd-experience-note" rows="4" placeholder="Tell the host anything important about your visit..."></textarea></div></div><div class="vd-booking-summary"><div><span>Village</span><b>${esc(v.name)}</b></div><div><span>Duration</span><b>${esc(e[2])}</b></div><div><span>Price</span><b>₹${esc(e[3])} / person</b></div></div><div class="menu-actions"><button class="primary" data-action="submit-experience-booking" data-village="${esc(v.id)}" data-experience="${esc(e[1])}">📩 Request Booking</button></div>`);
}
async function submitExperienceBooking(villageId,experienceName){
 if(!(await requireGoogleAuth()))return;
 const v=villages.find(x=>x.id===villageId);if(!v)return;
 const e=getVillageExperiences(v).find(x=>x[1]===experienceName);if(!e)return;
 const date=document.getElementById('vd-experience-date')?.value||'';
 if(!date){openModal('Choose a date','Please select the date you want to experience this activity.');return;}
 const guests=Math.max(1,Math.min(20,Number(document.getElementById('vd-experience-guests')?.value||2)));
 const ownerUid=v.ownerUid||v.listing?.ownerUid||'';
 const id=`${firebaseUser.uid}_experience_${Date.now()}`;
 const booking={id,type:'experience',uid:firebaseUser.uid,userName:firebaseUser.displayName||'VillageDeko User',userEmail:firebaseUser.email||'',ownerUid,villageId:v.id,villageName:v.name,district:v.district,state:v.state,experience: e[1],duration:e[2],price:Number(e[3]||0),guests,date,note:document.getElementById('vd-experience-note')?.value?.trim()||'',status:'pending',createdAt:Date.now()};
 const local=[...vdUserArray('villagedeko_bookings'),booking];vdSetUserArray('villagedeko_bookings',local);let savedRemote=false;
 if(firestoreDb){try{await window.VD_FIREBASE.setDoc(window.VD_FIREBASE.doc(firestoreDb,'bookings',id),booking);savedRemote=true;}catch(err){console.warn('Experience booking save failed',err);}}
 if(ownerUid&&ownerUid!==firebaseUser.uid)await vdCreateNotification(ownerUid,`New experience booking for ${v.name}`,`${booking.experience} · ${booking.date} · ${booking.guests} guest(s).`);
 openModal(savedRemote?'Experience Booking Request Sent':'Experience Booking Request Saved',savedRemote?`Your request for ${e[1]} in ${v.name} has been sent to the host. Status: Pending.`:`Your request is saved on this device. Firebase could not be reached, so the host may not see it yet.`);
}

function renderStories(filter='all'){const grid=document.getElementById('story-grid');if(!grid)return;let list=currentVillage?stories.filter(s=>s.villageId===currentVillage.id):stories;if(filter!=='all')list=list.filter(s=>s.category===filter);grid.innerHTML=list.length?list.map((s)=>{const i=stories.indexOf(s);return `<article class="story-card" data-category="${s.category}" data-action="story" data-story="${i}" role="button" tabindex="0"><img src="${s.img}" alt="${s.tag}"><div class="story-card-body"><span>${s.tag}</span><h3>${s.title}</h3><p>${s.person}</p><button class="primary" data-action="story" data-story="${i}">Read Story</button></div></article>`}).join(''):`<div class="empty-state"><strong>${currentVillage?`No stories from ${esc(currentVillage.name)} yet.`:'No village stories yet.'}</strong><p>${currentVillage?'Stories from this village will appear here as the village is built out.':'Stories from villages across India will appear here.'}</p></div>`;}
...
function renderMarket(category='all'){
 const cats=[['all','🛍️ All Products'],['Food & Grains','🌾 Food & Grains'],['Honey','🍯 Honey'],['Spices','🌶️ Spices'],['Handicrafts','🧺 Handicrafts'],['Local Art','🎨 Local Art'],['Textiles','👗 Textiles'],['Handmade Products','🪔 Handmade Products']];
 const cr=document.getElementById('market-categories');
 if(cr)cr.innerHTML=cats.map(([key,label])=>`<button class="market-category ${key===category?'active':''}" data-action="market-category" data-category="${esc(key)}">${label}</button>`).join('');
 const grid=document.getElementById('market-grid');
 if(!grid)return;
 const source=Array.isArray(marketplaceProducts)?marketplaceProducts:products; const list=category==='all'?source.map((p,i)=>[p,i]):source.map((p,i)=>[p,i]).filter(([p])=>p.category===category);
 grid.innerHTML=list.map(([p,i])=>`<article class="market-product-card">
   <img src="${esc(p.image)}" alt="${esc(p.name)}">
   <div class="market-product-body">
    <span class="market-product-category">${esc(p.category)}</span>
    <h3>${esc(p.name)}</h3>
    <strong class="market-product-price">₹${Number(p.price).toLocaleString('en-IN')}</strong>
    <p class="market-product-seller"><b>${esc(p.seller)}</b><br>${esc(p.village)}</p>
    <p class="market-product-story">${esc(p.story)}</p>
    <button class="primary market-view-btn" data-action="product" data-product="${i}">View Product</button>
   </div>
 </article>`).join('');
}
function openMarketProduct(index){
 const p=(marketplaceProducts||products)[Number(index)];
 if(!p)return;
 openModal(p.name,`<div class="market-product-detail">
  <img src="${esc(p.image)}" alt="${esc(p.name)}">
  <span class="market-product-category">${esc(p.category)}</span>
  <h4>${esc(p.name)}</h4>
  <strong class="market-product-detail-price">₹${Number(p.price).toLocaleString('en-IN')}</strong>
  <p><b>${esc(p.seller)}</b><br>${esc(p.village)}</p>
  <div class="market-story-box"><b>Story</b><p>${esc(p.story)}</p></div>
  <button class="primary market-buy-btn" data-action="buy-product" data-product="${Number(index)}">Buy</button>
 </div>`);
}
function openMarketBuy(index){
 const p=(marketplaceProducts||products)[Number(index)];
 if(!p)return;
 openModal(`Buy — ${p.name}`,`<div class="market-buy-detail">
  <p><b>${esc(p.name)}</b> · ₹${Number(p.price).toLocaleString('en-IN')}</p>
  <p>${esc(p.seller)} · ${esc(p.village)}</p>
  <p class="form-note">The product detail and Buy flow are ready in the marketplace UI. A real checkout/payment link is not present in the supplied code, so no external purchase destination has been invented.</p>
 </div>`);
}
function renderProfile(){document.getElementById('profile-root').innerHTML=profile6(currentVillage)}
renderStates();renderVillages();renderExperiences();renderTrips();renderStories();renderMarket();initBazaarProductForm();loadMarketplaceProducts();if(currentVillage)renderProfile();
syncSavedListingIntoVillages();
renderVillageFeed();
...
document.addEventListener('click',async e=>{const el=e.target.closest('[data-action],[data-nav],[data-state],[data-filter],[data-story-filter],[data-p],[data-profile-visit],[data-profile-tab],[data-media-index]');if(!el)return;if(el.dataset.nav){showSlide(el.dataset.nav);return}if(el.dataset.state){renderVillages(el.dataset.state);return}if(el.dataset.filter){document.querySelectorAll('[data-filter]').forEach(x=>x.classList.remove('active'));el.classList.add('active');renderExperiences(el.dataset.filter);return}if(el.dataset.action==='filter-ek-din'){document.querySelectorAll('[data-filter]').forEach(b=>b.classList.toggle('active',b.dataset.filter==='ek-din'));renderExperiences('ek-din');return}if(el.dataset.action==='ek-din-slot'){const i=Number(el.dataset.ekdin),x=ekDinGaonMein[i];if(!x)return;document.querySelectorAll('.ek-din-slot').forEach(b=>b.classList.toggle('active',b===el));const d=document.getElementById('ek-din-detail');if(d)d.innerHTML=`<span>${x[0]}</span><h4>${x[1]}</h4><p>${x[2]}</p><b>📍 ${currentVillage?`${currentVillage.name} · ${currentVillage.district} · ${currentVillage.state}`:'India · Multiple villages'}</b>`;return}if(el.dataset.storyFilter){document.querySelectorAll('.story-filter').forEach(x=>x.classList.remove('active'));el.classList.add('active');renderStories(el.dataset.storyFilter);return}if(el.dataset.p){document.querySelectorAll('.profile-tab').forEach(x=>x.classList.remove('active'));el.classList.add('active');document.getElementById('profile-pages').innerHTML=profilePage(+el.dataset.p,currentVillage);return}if(el.dataset.profileVisit){const b=document.querySelector('.profile-tab[data-p="7"]');b?.click();return}if(el.dataset.profileTab){const b=document.querySelector(`.profile-tab[data-p="${el.dataset.profileTab}"]`);b?.click();return}if(el.closest('.profile-media-viewer'))return;const mediaTarget=el.closest('.profile-cover-media[data-media-index],.profile-media-item[data-media-index]');if(mediaTarget&&currentVillage){openProfileMediaViewer(Number(mediaTarget.dataset.mediaIndex||0));return}const a=el.dataset.action;if(a==='close'){closeModal();return}if(a==='explore'||a==='all-villages'){showSlide('slide-explore');return}if(a==='marketplace'){showSlide('slide-market');renderMarket();return}if(a==='bazaar-form'){openBazaarProductForm();return}if(a==='save-bazaar-product'){await saveBazaarProduct();return}if(a==='back-to-market'){showSlide('slide-market');return}if(a==='back-to-explore'){showSlide('slide-explore');return}if(a==='market-category'){renderMarket(el.dataset.category||'all');return}if(a==='all-stories'){showSlide('slide-stories');return}if(a==='back-to-stories'){closeModal();showSlide('slide-stories');return}if(a==='state-prev'){scrollByCard('state-slider',-1);return}if(a==='state-next'){scrollByCard('state-slider',1);return}if(a==='village-prev'){scrollByCard('village-grid',-1);return}if(a==='village-next'){scrollByCard('village-grid',1);return}if(a==='village'){setCurrentVillage(villages.find(v=>v.id===el.dataset.id)||null);renderProfile();renderExperiences();renderTrips();renderStories();showSlide('slide-profile');return}if(a==='random'){setCurrentVillage(villages[Math.floor(Math.random()*villages.length)]||null);renderProfile();renderExperiences();renderTrips();renderStories();showSlide('slide-profile');return}if(a==='menu'){openRoleMenu();return}if(a==='choose-user'){if(!(await requireGoogleAuth()))return;setUserRole('user');closeModal();openRoleMenu();return}if(a==='choose-host'){if(!(await requireGoogleAuth()))return;setUserRole('host');closeModal();openRoleMenu();return}if(a==='switch-user'){if(!(await requireGoogleAuth()))return;setUserRole('user');closeModal();openRoleMenu();return}if(a==='switch-host'){if(!(await requireGoogleAuth()))return;setUserRole('host');closeModal();openRoleMenu();return}if(a==='listing'||a==='host'){if(!(await requireGoogleAuth()))return;if(getUserRole()!=='host'){openRoleChooser();return}showSlide('slide-listing');closeModal();return}if(a==='google-signin'){if(firebaseUser)openModal('Google Account',`Signed in as <b>${esc(firebaseUser.displayName||firebaseUser.email||'Google user')}</b><br><br><button class="secondary" data-action="google-signout">Sign out</button>`);else signInGoogle();return}if(a==='google-signout'){window.VD_FIREBASE?.signOut(firebaseAuth).then(()=>{localStorage.removeItem('villagedeko_user_role');applyUserRoleUI();openModal('Signed out','Signed out successfully. Google sign-in is required to create or use a VillageDeko profile.');}).catch(console.warn);return}if(a==='profile'){openUserProfile();return}if(a==='host-dashboard'){if(!(await requireGoogleAuth()))return;openHostDashboard();return}if(a==='save-village'){toggleSaveVillage();return}if(a==='settings'){openModal('Settings','Account, privacy, notifications and app settings will appear here.');return}if(a==='search'){showSlide('slide-explore');setTimeout(()=>{const q=document.getElementById('village-search');q?.focus();q?.scrollIntoView({behavior:'smooth',block:'center'});},80);return}if(a==='photos'){openModal('Village Photos','The host can keep one separate cover photo and add multiple gallery photos for this village.');return}if(a==='videos'){openModal('Village Videos','Hosts can add village videos later. Videos will appear inside the village experience.');return}if(a==='plan'){await openTripPlanner();return}if(a==='reserve'){await openVillageBooking();return}if(a==='book-experience'){await openExperienceBooking(el.dataset.village||'',el.dataset.experience||'');return}if(a==='submit-experience-booking'){await submitExperienceBooking(el.dataset.village||'',el.dataset.experience||'');return}if(a==='book'){await openVillageBooking();return}if(a==='trip-current'){if(currentVillage)showSlide('slide-stay');else showSlide('slide-explore');return}if(a==='book-trip'){const tripIndex=Number(el.dataset.trip||0);let bookingIndex=Number.isFinite(tripIndex)?tripIndex:0;if(!currentVillage){const entry=getTrips()[tripIndex];if(entry?.village){setCurrentVillage(entry.village);const selectedTrips=getTrips();bookingIndex=Math.max(0,selectedTrips.findIndex(t=>t.name===entry.trip.name));renderProfile();renderExperiences();renderTrips();renderStories();}}await openVillageBooking(bookingIndex);return}if(a==='story'){openStoryReader(+(el.dataset.story||0));return}if(a==='story-prev'){scrollByCard('story-grid',-1);return}if(a==='story-next'){scrollByCard('story-grid',1);return}if(a==='food-journey-open'){renderFoodJourneyModal();return}if(a==='food-journey-stage'||a==='food-journey-stage-modal'){const stages=[['🌱','Beej','Sahi beej se kheti ka safar shuru hota hai.'],['🌾','Khet / Buwai','Beej mitti mein boya jaata hai aur fasal ugne lagti hai.'],['💧','Paani & Dekhbhal','Paani, mitti aur fasal ki dekhbhal zaroori hoti hai.'],['🌿','Fasal','Fasal pakti hai aur katai ke liye taiyaar hoti hai.'],['🌾','Katai','Pakki fasal ko khet se kaat kar jama kiya jaata hai.'],['🏪','Mandi','Fasal mandi tak pahunchti hai aur aage buyers/processors tak jaati hai.'],['⚙️','Processing','Anaj ya ingredient ko saaf, peesa ya process kiya jaata hai.'],['🏠','Ghar','Food ghar tak pahunchta hai aur rasoi mein taiyaar hota hai.'],['🍞','Plate','Aakhir mein poora safar hamari plate par complete hota hai.']];const i=Number(el.dataset.stage||0),x=stages[i];if(!x)return;const id=a==='food-journey-stage'?'food-journey-detail':'food-journey-modal-detail';const d=document.getElementById(id);if(d){d.innerHTML=`<h4>${x[1]}</h4><p>${x[2]}</p>`;}return}if(a==='foodstory'){openModal('Local Food Story','Follow a real ingredient from seed and field through harvest, mandi, processing and finally the local plate.');return}if(a==='product'){openMarketProduct(el.dataset.product||0);return}if(a==='buy-product'){openMarketBuy(el.dataset.product||0);return}if(a==='listing-place'){const d=currentVillage.listing;const x=d?.explore?.[+(el.dataset.index||0)];if(x)openModal(x[0]||'Village Place',`<b>${esc(x[1]||'')}</b><p>${esc(x[2]||'')}</p>`);return}if(a==='listing-experience'){const x=currentVillage.listing?.experience?.[+(el.dataset.index||0)];if(x)openModal(x[0]||'Experience',`${rowMediaMarkup(x,5,6,x[0]||'Experience')}<b>${esc(x[1]||'')}</b><p>${esc(x[4]||'')}</p><p>${esc(x[2]||'')} · ₹${esc(x[3]||'0')} / person</p>`);return}if(a==='listing-journey'){const x=currentVillage.listing?.journey?.[+(el.dataset.index||0)];if(x)openModal(x[0]||'Food Journey Stage',`${rowMediaMarkup(x,4,5,x[0]||'Food Journey')}<p>${esc(x[1]||'')} → ${esc(x[2]||'')}</p><p>${esc(x[3]||'')}</p>`);return}if(a==='listing-story'){const x=currentVillage.listing?.people?.[+(el.dataset.index||0)];if(x)openModal(x[2]||x[0]||'Village Story',`${rowMediaMarkup(x,4,5,x[0]||'Village Story')}<b>${esc(x[0]||'')} · ${esc(x[1]||'')}</b><p>${esc(x[3]||'')}</p>`);return}if(a==='place'){const place=el.dataset.place||'Village Section';openModal(place,`${place} — ${currentVillage.name}. Explore this part of the village through future real photos, videos, local stories and practical information.`);return}if(a==='cover'){openModal('Village Cover Photo','Cover photo is separate from the village gallery. In the connected form, the host will select one cover image.');return}if(a==='media'){openModal('Village Media','The host can add multiple photos and videos to the village profile.');return}if(a==='save-listing'){openModal('Village Listing Saved','Your village listing form is ready. Firebase will store the village details and Cloudinary will store media in the connected build.');return}});
const listingFormState={step:1,total:11,initialized:false};
const listingTemplates={
 explore:[['Place name','text','e.g. Village temple'],['Category','select',['Khet','Ghar','Dairy','Mandir','Market','Nature','Craft','Other']],['Description','textarea','What can visitors discover here?'],['Place photo','image',''],['Place video','video','']],
 experience:[['Experience name','text','e.g. Farm Visit'],['Category','select',['Activities','Food','Culture','Nature','Farming','Craft','Other']],['Duration','text','e.g. 2 Hours'],['Price per person *','number','₹'],['Description','textarea','What will visitors do?'],['Experience photo *','image',''],['Experience video (optional)','video','']],
 day:[['Time','text','e.g. 05:30 AM'],['Activity','text','e.g. Gaon ki subah'],['Description','textarea','What happens at this time?'],['Moment photo','image',''],['Moment video (optional)','video',''],['Local voice (optional)','audio',''],['Local person / storyteller','text','e.g. Ramesh ji · Farmer']],
 food:[['Dish name','text','e.g. Bajre ki roti'],['Type','text','e.g. Breakfast / Lunch'],['Description','textarea','What makes this food local?'],['Dish photo *','image',''],['Dish video (optional)','video',''],['Price per person *','number','₹']],
 journey:[['Stage name','text','e.g. Beej / Khet / Katai'],['From','text','Where does this stage start?'],['To','text','Where does it go next?'],['Description','textarea','Explain this part of the food journey.'],['Stage photo','image',''],['Stage video (optional)','video',''],['Farmer / local person','text','e.g. Ramesh ji · Farmer'],['Local voice (optional)','audio','']],
 people:[['Person name','text','e.g. Gopal Singh'],['Role','text','e.g. Farmer / Artisan'],['Story title','text','e.g. 40 years of farming'],['Story','textarea','Tell their story.'],['Person / story photo','image',''],['Person / story video (optional)','video',''],['Meri Kahani voice (optional)','audio','']],
 culture:[['Culture item','text','e.g. Gangaur Festival'],['Type','select',['Festival','Tradition','Folk Music','Dance','Craft','Marriage','Other']],['Description','textarea','Explain the local tradition.'],['Culture photo','image',''],['Culture video','video','']],
 package:[['Package name','text','e.g. 2-Day Complete Village Experience'],['Duration','text','e.g. 2 Days / 1 Night'],['Itinerary / experience summary','textarea','Day 1..., Day 2...'],['Included experiences','textarea','Comma separated: Farm Visit, Dairy, Local Food'],['Stay included','text','e.g. 1 night homestay'],['Meals included','text','e.g. Breakfast, lunch & dinner'],['Host / guide','text','e.g. Local village host'],['Adult price per person *','number','₹'],['Child price','number','₹'],['Minimum guests','number','1'],['Maximum guests','number','10'],['Availability','text','e.g. October – March'],['Not included','textarea','Comma separated: Transport, shopping, insurance'],['Package photo *','image',''],['Package video (optional)','video','']]
};
...
function listingFieldHtml(def){const [label,type,extra]=def;if(type==='select')return `<label>${label}<select>${extra.map(x=>`<option>${x}</option>`).join('')}</select></label>`;if(['image','video','audio'].includes(type)){const accept=type==='video'?'video/*':type==='audio'?'audio/*':'image/*',icon=type==='video'?'▶':type==='audio'?'🎙':'▧';return `<label class="full upload-card">${label}<input type="file" accept="${accept}" data-file-field="1"><span>${icon}</span><strong>Choose ${type==='audio'?'voice/audio':type}</strong><small>Optional ${type==='audio'?'local voice':type} for this exact village item</small></label>`}return `<label>${label}<${type==='textarea'?'textarea':'input'} ${type!=='textarea'?`type="${type}"`:''} placeholder="${extra||''}" ${type==='textarea'?'rows="3"':''}></${type==='textarea'?'textarea':'input'}></label>`}
function addListingRow(type,values={}){const root=document.getElementById(`${type}-repeat`);if(!root)return;const defs=listingTemplates[type];const card=document.createElement('div');card.className='repeat-card';card.dataset.repeat=type;card.innerHTML=`<div class="repeat-card-head"><strong>${type==='explore'?'Explore place':type==='experience'?'Experience':type==='day'?'Day time slot':type==='food'?'Local dish':type==='journey'?'Food Journey stage':type==='people'?'Person & Story':type==='package'?'Complete Village Package':'Culture item'}</strong><button type="button" class="remove-row">Remove</button></div><div class="repeat-grid">${defs.map((d,i)=>listingFieldHtml(d)).join('')}</div>`;root.appendChild(card);const fields=card.querySelectorAll('input,select,textarea');fields.forEach((f,i)=>{if(f.type==='file'){if(values[i]){f.dataset.existing=typeof values[i]==='string'?values[i]:values[i]?.url||'';const note=document.createElement('small');note.className='form-note existing-file-note';note.textContent=`Current ${f.accept?.startsWith('video')?'video':'photo'} saved. Choose a new file only if you want to replace it.`;f.parentElement.appendChild(note);}return;}if(values[i]!=null)f.value=values[i]});card.querySelector('.remove-row').addEventListener('click',()=>card.remove())}
...
function fillListingFormFromSaved(d){
 ...
 Object.keys(d).forEach(k=>{if(listingTemplates[k]||k==='coverPhoto'||k==='galleryPhotos'||k==='galleryVideos'||k==='stayPhoto'||k==='stayVideo')return;const el=f.elements[k];if(el&&typeof d[k]==='string')el.value=d[k]});
 const stayInput=f.elements.stayPhoto;
 if(stayInput&&d.stayPhoto){stayInput.dataset.existing=typeof d.stayPhoto==='string'?d.stayPhoto:(d.stayPhoto.url||'');if(stayInput.dataset.existing){const note=document.createElement('small');note.className='form-note existing-file-note';note.textContent='Current stay photo saved. Choose a new photo only if you want to replace it.';stayInput.parentElement.appendChild(note);}}
 const stayVideoInput=f.elements.stayVideo; if(stayVideoInput&&d.stayVideo){stayVideoInput.dataset.existing=typeof d.stayVideo==='string'?d.stayVideo:(d.stayVideo.url||'');if(stayVideoInput.dataset.existing){const note=document.createElement('small');note.className='form-note existing-file-note';note.textContent='Current stay video saved. Choose a new video only if you want to replace it.';stayVideoInput.parentElement.appendChild(note);}}
...
async function saveVillageListing(){const f=document.getElementById('village-listing-form');if(!f.checkValidity()){f.reportValidity();return}const data=getListingData(),savedListing=getSavedListing()||{};
 const cover=f.elements.coverPhoto;if(!cover?.files?.[0]&&!savedListing.cover&&!savedListing.media?.cover){goListingStep(2);openModal('Village cover photo required','Village profile ke liye ek clear cover photo add karna zaroori hai.');return;}
 const validateRows=(selector,step,priceIndex,photoIndex,label)=>{for(const row of document.querySelectorAll(selector)){const fields=[...row.querySelectorAll('input,select,textarea')],name=(fields[0]?.value||'').trim();if(!name)continue;const price=Number(fields[priceIndex]?.value||0),photo=fields[photoIndex];if(!(price>0)){goListingStep(step);openModal(`${label} price required`,`${name} ke liye valid price add karein.`);return false}if(!photo?.files?.[0]&&!photo?.dataset.existing){goListingStep(step);openModal(`${label} photo required`,`${name} ke booking card ke liye actual photo add karna zaroori hai.`);return false}}return true};
 if(!validateRows('#experience-repeat .repeat-card',4,3,5,'Experience'))return;
 if(!validateRows('#food-repeat .repeat-card',6,5,3,'Food'))return;
 if(!validateRows('#package-repeat .repeat-card',10,7,13,'Package'))return;
 if(f.elements.stayAvailable?.value==='Yes'){const stay=f.elements.stayPhoto,price=Number(f.elements.stayPrice?.value||0);if(!(price>0)){goListingStep(10);openModal('Stay price required','Stay available hai, isliye valid per-night price add karein.');return}if(!stay?.files?.[0]&&!stay?.dataset.existing&&!savedListing.stayPhoto&&!savedListing.media?.stay){goListingStep(10);openModal('Stay photo required','Stay available hai, isliye actual stay / homestay ka photo add karna zaroori hai.');return;}}
...
async function openVillageBooking(preselectedTripIndex=null){
 if(!(await requireGoogleAuth()))return;
 const v=currentVillage;if(!v)return;const trips=getTrips();
 const defaultIndex=Number.isInteger(preselectedTripIndex)&&preselectedTripIndex>=0&&preselectedTripIndex<trips.length?preselectedTripIndex:0;
 const today=new Date().toISOString().slice(0,10);
 openModal('Visit This Village',`<div class="vd-booking-header"><div class="vd-feature-card"><strong>${esc(v.name)}</strong><small>📍 ${esc(v.district)} · ${esc(v.state)}</small></div><p class="vd-booking-note">Choose a village package, date and guests. Your request will go to the host for confirmation.</p></div><div class="vd-modal-form vd-booking-form"><div class="vd-form-field vd-form-field-full"><label for="vd-booking-trip">Village experience / package</label><select id="vd-booking-trip">${trips.map((t,i)=>`<option value="${i}" ${i===defaultIndex?'selected':''}>${esc(t.name)} · ${esc(t.duration)} · ₹${Number(t.price||0).toLocaleString('en-IN')}</option>`).join('')}</select></div><div class="vd-form-field"><label for="vd-booking-date">Visit date</label><input id="vd-booking-date" type="date" min="${today}"></div><div class="vd-form-field"><label for="vd-booking-guests">Adults</label><input id="vd-booking-guests" type="number" min="1" max="20" value="2"></div><div class="vd-form-field"><label for="vd-booking-children">Children</label><input id="vd-booking-children" type="number" min="0" max="20" value="0"></div><div class="vd-form-field vd-form-field-full"><label for="vd-booking-note">Message to host <span class="vd-optional">(optional)</span></label><textarea id="vd-booking-note" rows="4" placeholder="Tell the host anything important about your visit..."></textarea></div></div><div id="vd-booking-summary" class="vd-booking-summary"></div><div class="menu-actions"><button class="primary" data-action="submit-village-booking">📩 Request Booking</button></div>`);
 renderVillageBookingSummary();
}
function renderVillageBookingSummary(){
 const trips=getTrips(),i=Number(document.getElementById('vd-booking-trip')?.value||0),t=trips[i];const d=document.getElementById('vd-booking-date'),g=document.getElementById('vd-booking-guests'),root=document.getElementById('vd-booking-summary');
 if(!root||!t)return;root.innerHTML=`<div><span>Package</span><b>${esc(t.name)}</b></div><div><span>Stay</span><b>${esc(t.stay)}</b></div><div><span>Host</span><b>${esc(t.host)}</b></div><div><span>Listed price</span><b>₹${Number(t.price||0).toLocaleString('en-IN')}</b></div><div><span>Date</span><b>${d?.value||'Select a date'}</b></div><div><span>Guests</span><b>${Number(g?.value||2)} adults · ${Number(document.getElementById('vd-booking-children')?.value||0)} children</b></div><div><span>Estimated total</span><b>₹${(Number(t.price||0)*Number(g?.value||2)+Number(t.childPrice||t.price||0)*Number(document.getElementById('vd-booking-children')?.value||0)).toLocaleString('en-IN')}</b></div><p>Final availability and any transport/details will be confirmed by the host.</p>`;
}
async function submitVillageBooking(){
 if(!(await requireGoogleAuth()))return;const v=currentVillage,trips=getTrips(),i=Number(document.getElementById('vd-booking-trip')?.value||0),t=trips[i];if(!v||!t)return;
 const date=document.getElementById('vd-booking-date')?.value||'';if(!date){openModal('Choose a date','Please select the date you want to visit the village.');return;}
 const guests=Math.max(1,Math.min(20,Number(document.getElementById('vd-booking-guests')?.value||2)));const children=Math.max(0,Math.min(20,Number(document.getElementById('vd-booking-children')?.value||0)));const ownerUid=v.ownerUid||v.listing?.ownerUid||'';const id=`${firebaseUser.uid}_${Date.now()}`;
 const booking={id,uid:firebaseUser.uid,userName:firebaseUser.displayName||'VillageDeko User',userEmail:firebaseUser.email||'',ownerUid,villageId:v.id,villageName:v.name,district:v.district,state:v.state,tripIndex:i,trip:t.name,duration:t.duration,stay:t.stay,price:Number(t.price||0),childPrice:Number(t.childPrice||t.price||0),guests,children,total:Number(t.price||0)*guests+Number(t.childPrice||t.price||0)*children,date,note:document.getElementById('vd-booking-note')?.value?.trim()||'',status:'pending',createdAt:Date.now()};
 const local=[...vdUserArray('villagedeko_bookings'),booking];vdSetUserArray('villagedeko_bookings',local);let savedRemote=false;
 if(firestoreDb){try{await window.VD_FIREBASE.setDoc(window.VD_FIREBASE.doc(firestoreDb,'bookings',id),booking);savedRemote=true;}catch(e){console.warn('Booking save failed',e);}}
 if(ownerUid&&ownerUid!==firebaseUser.uid)await vdCreateNotification(ownerUid,`New booking request for ${v.name}`,`${booking.trip} · ${booking.date} · ${booking.guests} guest(s).`);
 openModal(savedRemote?'Booking Request Sent':'Booking Request Saved',savedRemote?`Your booking request for ${v.name} has been sent to the host. Status: Pending.`:`Your booking request is saved on this device. Firebase could not be reached, so the host may not see it yet.`);
}
async function openMyBookings(){
 if(!(await requireGoogleAuth()))return;let list=vdUserArray('villagedeko_bookings').filter(x=>x.uid===firebaseUser.uid);
 if(firestoreDb){try{const q=window.VD_FIREBASE.query(window.VD_FIREBASE.collection(firestoreDb,'bookings'),window.VD_FIREBASE.where('uid','==',firebaseUser.uid));const snap=await window.VD_FIREBASE.getDocs(q);list=snap.docs.map(d=>d.data());vdSetUserArray('villagedeko_bookings',list);}catch(_e){}}
 const html=list.length?list.slice().reverse().map(x=>`<div class="vd-list-card"><strong>${esc(x.villageName)}</strong><p>${esc(x.trip)} · ${esc(x.duration)}</p><small>📅 ${esc(x.date)} · 👥 ${x.guests||1} · ₹${Number(x.price||0).toLocaleString('en-IN')}</small><div class="vd-booking-status vd-status-${esc(x.status||'pending')}">${esc((x.status||'pending').toUpperCase())}</div></div>`).join(''):'<p class="form-note">No village bookings yet.</p>';
 openModal('My Bookings',`<div class="vd-list">${html}</div>`);
}
async function openHostBookings(){
 if(!(await requireGoogleAuth()))return;if(getUserRole()!=='host'){openRoleChooser();return;}let list=vdUserArray('villagedeko_bookings').filter(x=>x.ownerUid===firebaseUser.uid);
 if(firestoreDb){try{const q=window.VD_FIREBASE.query(window.VD_FIREBASE.collection(firestoreDb,'bookings'),window.VD_FIREBASE.where('ownerUid','==',firebaseUser.uid));const snap=await window.VD_FIREBASE.getDocs(q);list=snap.docs.map(d=>d.data());}catch(_e){}}
 const html=list.length?list.slice().reverse().map((x,i)=>`<div class="vd-list-card"><strong>${esc(x.villageName)}</strong><p>${esc(x.trip)} · ${esc(x.userName||'Traveller')}</p><small>📅 ${esc(x.date)} · 👥 ${x.guests||1} · ₹${Number(x.price||0).toLocaleString('en-IN')}</small>${x.note?`<p>${esc(x.note)}</p>`:''}<div class="vd-booking-status vd-status-${esc(x.status||'pending')}">${esc((x.status||'pending').toUpperCase())}</div>${x.status==='pending'?`<div class="menu-actions"><button class="secondary" data-action="booking-status" data-index="${i}" data-status="accepted">Accept</button><button class="secondary" data-action="booking-status" data-index="${i}" data-status="rejected">Reject</button></div>`:''}</div>`).join(''):'<p class="form-note">No booking requests yet.</p>';
 window.__vdBookings=list;openModal('Village Bookings',`<div class="vd-list">${html}</div>`);
}
async function setBookingStatus(index,status){
 const x=window.__vdBookings?.[index];if(!x)return;x.status=status;const all=vdUserArray('villagedeko_bookings');const pos=all.findIndex(y=>y.id===x.id);if(pos>=0){all[pos]=x;vdSetUserArray('villagedeko_bookings',all);}
 if(x.id&&firestoreDb){try{await window.VD_FIREBASE.setDoc(window.VD_FIREBASE.doc(firestoreDb,'bookings',x.id),x,{merge:true});}catch(_e){}}await vdCreateNotification(x.uid,`Host ${status} your village booking`,`Your booking for ${x.villageName} on ${x.date} was ${status}.`);openModal('Booking Updated',`The booking was marked ${status}. The traveller has been notified.`);
}
...
function ensureFinalFeatureStyles(){if(document.getElementById('vd-final-feature-styles'))return;const s=document.createElement('style');s.id='vd-final-feature-styles';s.textContent=`
 ...
 .vd-booking-header{display:grid;gap:8px}.vd-booking-note{margin:0;color:#666;font-size:13px}.vd-booking-form{display:grid;grid-template-columns:1fr 1fr;gap:10px}.vd-booking-form .vd-form-field-full{grid-column:1/-1}.vd-booking-form input,.vd-booking-form select,.vd-booking-form textarea{width:100%;box-sizing:border-box;border:1px solid rgba(0,0,0,.14);border-radius:12px;padding:11px 12px;background:#fff;font:inherit}.vd-optional{font-weight:400;color:#888}.vd-booking-summary{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:12px;padding:12px;border-radius:14px;background:#f6f3ed}.vd-booking-summary>div{display:grid;gap:2px}.vd-booking-summary span{font-size:11px;color:#777}.vd-booking-summary b{font-size:13px}.vd-booking-summary p{grid-column:1/-1;margin:4px 0 0;color:#777;font-size:11px}.vd-booking-status{display:inline-block;margin-top:8px;padding:5px 9px;border-radius:999px;font-size:10px;font-weight:800;letter-spacing:.06em}.vd-status-pending{background:#fff3cd;color:#795500}.vd-status-accepted{background:#e5f6e9;color:#236b38}.vd-status-rejected{background:#fde8e8;color:#9b2c2c}
 @media(max-width:640px){.profile-action-row{display:grid;grid-template-columns:1fr 1fr}.profile-action-row .profile-save-village{grid-column:1/-1}.profile-action-row button{width:100%;font-size:13px}.vd-list{max-height:48vh}.vd-booking-form{grid-template-columns:1fr}.vd-booking-form .vd-form-field-full{grid-column:auto}}
 `;document.head.appendChild(s)}
ensureFinalFeatureStyles();

document.addEventListener('input',e=>{if(e.target.id==='vd-booking-date'||e.target.id==='vd-booking-guests')renderVillageBookingSummary();});
document.addEventListener('change',e=>{if(e.target.id==='vd-booking-trip')renderVillageBookingSummary();});
...
document.addEventListener('click',async e=>{
 ...
 if(a==='my-bookings'){await openMyBookings();return}
 if(a==='submit-village-booking'){await submitVillageBooking();return}
 if(a==='booking-status'){const b=e.target.closest('[data-action="booking-status"]');await setBookingStatus(Number(b?.dataset.index||0),b?.dataset.status||'accepted');return}
 if(a==='host-bookings'){await openHostBookings();return}
...
function ensureVillageFeedStyles(){
 ...
 .village-feed-media{width:100%;aspect-ratio:4/5;object-fit:cover;display:block;background:#eee}.village-feed-video{width:100%;aspect-ratio:16/10;object-fit:cover;display:block;background:#111}
...
function renderVillageFeed(){
 const root=document.getElementById('village-feed-list'),empty=document.getElementById('village-feed-empty');if(!root)return;
 const posts=getAllVillageFeedPosts();
 if(empty)empty.hidden=posts.length>0;
 if(!posts.length){root.innerHTML='';return;}
 const liked=firebaseUser?vdUserArray(`villagedeko_feed_likes_${firebaseUser.uid}`):[];
 const saved=vdUserArray('villagedeko_saved_feed_posts');
 root.innerHTML=posts.map(p=>{
  const media=p.media?.url||'';
  const isVideo=p.media?.resourceType==='video'||String(p.media?.type||'').startsWith('video/');
  const mediaHtml=media?(isVideo
   ?`<video class="village-feed-video" controls playsinline preload="metadata"><source src="${esc(media)}">Your browser does not support video playback.</video>`
   :`<button class="village-feed-media-btn" type="button" data-action="feed-media" data-post-id="${esc(p.id)}" aria-label="Open ${esc(p.villageName||'village')} photo"><img class="village-feed-media" src="${esc(media)}" alt="${esc(p.title||'Village update')}" loading="lazy"></button>`):'';
  const isLiked=liked.includes(p.id),isSaved=saved.some(x=>x.id===p.id);
  return `<article class="village-feed-card">
   <div class="village-feed-headrow">
    <button class="feed-village-link" type="button" data-action="feed-village" data-village-id="${esc(p.villageId||p.ownerUid||'')}">${esc(p.villageName||'Village')}</button>
    ${firebaseUser&&p.ownerUid===firebaseUser.uid?`<button class="feed-delete-icon" type="button" data-action="delete-feed-post" data-post-id="${esc(p.id)}" aria-label="Delete post" title="Delete post">⋯</button>`:''}
    <small>${esc(p.district||'')}${p.district&&p.state?' · ':''}${esc(p.state||'')}</small>
    <span>${esc(p.authorName||'Village Host')}</span>
   </div>
   ${mediaHtml}
   <div class="village-feed-body">
    ${p.title?`<h3>${esc(p.title)}</h3>`:''}
    ${p.text?`<p>${esc(p.text)}</p>`:''}
    <div class="village-feed-actions" aria-label="Post actions">
     <button class="${isLiked?'active':''}" data-action="feed-like" data-post-id="${esc(p.id)}" aria-label="Like" title="Like">${isLiked?'♥':'♡'}</button>
     <button data-action="feed-comment" data-post-id="${esc(p.id)}" aria-label="Comment" title="Comment">◌</button>
     <button class="${isSaved?'active':''}" data-action="feed-save" data-post-id="${esc(p.id)}" aria-label="Save" title="Save">${isSaved?'▮':'▯'}</button>
     <button data-action="feed-share" data-post-id="${esc(p.id)}" aria-label="Share" title="Share">↗</button>
    </div>
   </div>
  </article>`;
 }).join('');
}
function openFeedMedia(postId){
 const p=findFeedPost(postId);if(!p?.media?.url)return;
 const isVideo=p.media?.resourceType==='video'||String(p.media?.type||'').startsWith('video/');
 if(isVideo)return;
 openModal(esc(p.villageName||'Village Photo'),'',`<div class="feed-photo-viewer"><img src="${esc(p.media.url)}" alt="${esc(p.title||'Village photo')}"></div>`);
}
...
async function feedLike(id){if(!(await requireGoogleAuth()))return;const key=`villagedeko_feed_likes_${firebaseUser.uid}`,liked=vdUserArray(key),has=liked.includes(id),next=has?liked.filter(x=>x!==id):[...liked,id];vdSetUserArray(key,next);renderVillageFeed();if(firestoreDb){try{await window.VD_FIREBASE.setDoc(window.VD_FIREBASE.doc(firestoreDb,'feedInteractions',`${firebaseUser.uid}_${id}`),{uid:firebaseUser.uid,postId:id,liked:!has,saved:vdUserArray('villagedeko_saved_feed_posts').some(x=>x.id===id),updatedAt:Date.now()},{merge:true});}catch(e){console.warn('Feed like sync failed',e);}}}
async function feedComment(id){if(!(await requireGoogleAuth()))return;const p=findFeedPost(id);if(!p)return;const key=`villagedeko_feed_comments_${id}`;let comments=vdUserArray(key);if(firestoreDb){try{const q=window.VD_FIREBASE.query(window.VD_FIREBASE.collection(firestoreDb,'feedComments'),window.VD_FIREBASE.where('postId','==',id));const snap=await window.VD_FIREBASE.getDocs(q);comments=snap.docs.map(d=>d.data()).sort((a,b)=>Number(a.createdAt||0)-Number(b.createdAt||0));vdSetUserArray(key,comments);}catch(e){console.warn('Feed comments load failed',e);}}const list=comments.length?`<div class="vd-feed-comment-list">${comments.map(x=>`<article><strong>${esc(x.userName||'VillageDeko User')}</strong><p>${esc(x.text||'')}</p></article>`).join('')}</div>`:'<p class="form-note">Abhi koi comment nahi hai.</p>';openModal('Comment on Village Post',`<strong>${esc(p.title)}</strong>${list}<label>Comment<textarea id="vd-feed-comment" rows="4" maxlength="500" placeholder="Apni baat likho..."></textarea></label><div class="menu-actions"><button class="primary" data-action="submit-feed-comment" data-post-id="${esc(id)}">Post Comment</button></div>`)}
...
async function feedSave(id){if(!(await requireGoogleAuth()))return;const p=findFeedPost(id);if(!p)return;const key='villagedeko_saved_feed_posts',saved=vdUserArray(key),has=saved.some(x=>x.id===id),next=has?saved.filter(x=>x.id!==id):[...saved,{...p,savedAt:Date.now()}];vdSetUserArray(key,next);renderVillageFeed();if(firestoreDb){try{await window.VD_FIREBASE.setDoc(window.VD_FIREBASE.doc(firestoreDb,'feedInteractions',`${firebaseUser.uid}_${id}`),{uid:firebaseUser.uid,postId:id,liked:vdUserArray(`villagedeko_feed_likes_${firebaseUser.uid}`).includes(id),saved:!has,updatedAt:Date.now()},{merge:true});}catch(e){console.warn('Feed save sync failed',e);}}}
...
renderVillageFeed();
openRoleMenu=function(){const role=getUserRole();if(!role){openRoleChooser();return;}const isHost=role==='host';openModal(isHost?'Host Account':'User Account',isHost?'Manage your village and also explore other villages.':'Explore villages, plan trips and discover local experiences.',`<div class="menu-actions"><button class="secondary" data-action="profile">👤 My Profile</button><button class="secondary" data-action="my-trips">🗺️ My Trips</button><button class="secondary" data-action="my-bookings">🧳 My Bookings</button><button class="secondary" data-action="notifications">🔔 Notifications</button><button class="secondary" data-action="village-reviews">⭐ Reviews</button>${isHost?'<button class="secondary" data-action="listing">🏡 My Village Listing</button><button class="secondary" data-action="post-feed">📸 Post to Village Feed</button><button class="secondary" data-action="host-enquiries">📩 Enquiries</button><button class="secondary" data-action="host-bookings">🧳 Bookings</button><button class="secondary" data-action="switch-user">Switch to User</button>':'<button class="secondary" data-action="switch-host">Become a Host</button>'}<button class="secondary" data-action="settings">⚙️ Settings</button></div>`);};
