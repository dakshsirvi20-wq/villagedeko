...
renderStates();populateStateFilter();renderVillages();renderExperiences();renderTrips();renderStories();renderMarket();initBazaarProductForm();loadMarketplaceProducts();if(currentVillage)renderProfile();
syncSavedListingIntoVillages();
renderVillageFeed();
...
async function feedLike(id){if(!(await requireGoogleAuth()))return;const key=`villagedeko_feed_likes_${firebaseUser.uid}`,liked=vdUserArray(key),has=liked.includes(id);vdSetUserArray(key,has?liked.filter(x=>x!==id):[...liked,id]);}
...
async function feedSave(id){if(!(await requireGoogleAuth()))return;const p=findFeedPost(id);if(!p)return;const key='villagedeko_saved_feed_posts',saved=vdUserArray(key),has=saved.some(x=>x.id===id);vdSetUserArray(key,has?saved.filter(x=>x.id!==id):[...saved,{...p,savedAt:Date.now()}]);}
