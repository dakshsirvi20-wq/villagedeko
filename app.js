...
function openChaupal(){ if(!requireLogin())return; showView("chaupalView"); renderPosts(allPosts,$("chaupalFeedContainer")); }
...
async function loadHome(){
  renderVillageCards(villages,$("stateVillages"));
  $("villageCount").textContent=villages.length;
  renderPosts(allPosts,$("homePostsFeed"));
  $("postCount").textContent=allPosts.length;
}

function filterExplore(term){
  const q=term.trim().toLowerCase();
  if(!q){ renderPosts(allPosts,$("homePostsFeed")); return; }
  const matchingPosts=allPosts.filter(p=>{
    const v=findVillageForPost(p);
    const hay=[p.author,p.text,p.location,p.villageName,p.vDistrict,p.vState,v?.vName,v?.vDistrict,v?.vState,v?.vDescription,...(v?.activities||[]).map(a=>a.name)].join(" ").toLowerCase();
    return hay.includes(q);
  });
  renderPosts(matchingPosts,$("homePostsFeed"));
}
...
async function selectState(state){
  ...
  renderVillageCards(list,$("stateVillages"));
  const posts=allPosts.filter(p=>{const v=findVillageForPost(p);return (v?.vState||p.vState)===state;});
...
async function toggleLike(id,event){
  ...
  renderPosts(allPosts,$("homePostsFeed"));
  renderPosts(allPosts,$("chaupalFeedContainer"));
}
...
function openPostModal(){
  if(!requireLogin())return;
  openModal("addStoryModal");populateVillageSelect();$("storyAuthor").value=currentUser.displayName||"";
}
...
async function handleImgBBPhotoPost(event){
  ...
  try{
    ...
    if(!village)throw new Error("Village select karein.");
    await addDoc(collection(db,"posts"),{
      ownerUid:uid(),author:currentUser.displayName||$("storyAuthor").value.trim(),
      location:$("storyLocation").value.trim()||village.vName,
      villageId:village.id,villageName:village.vName,vDistrict:village.vDistrict,vState:village.vState,
      text:$("storyText").value.trim(),imageUrl,createdAt:serverTimestamp()
    });
    $("chaupalPostForm").reset();closeModal("addStoryModal");
    await loadAllPosts();
    if(currentVillage?.id===village.id)await loadVillagePosts();
    loadHome();
...
function updateAuthUI(){const logged=!!currentUser;$("drawerName").textContent=logged?(currentUser.displayName||"VillageDeko User"):"Guest";$("drawerEmail").textContent=logged?(currentUser.email||""):"";}
