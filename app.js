...
function openModal(title,text,extra=''){modalContent.innerHTML=`<div class="modal-scroll"><h3>${title}</h3><p>${text}</p>${extra}</div><div class="modal-footer"><button class="primary" data-action="close">Done</button></div>`;modal.classList.add('show')}
...
function profile6(v){return `<div class="profile-backbar"><button class="back-btn" data-action="back-to-explore">← Back to Explore</button></div><div class="profile-banner"><img src="${esc(v.cover)}" alt="${esc(v.name)}"><div class="profile-overlay"><span class="eyebrow">VILLAGE PROFILE</span><h2>${esc(v.name)}</h2><p>${esc(v.district)} · ${esc(v.state)}</p><button class="primary profile-save-village" data-action="save-village">❤️ Save Village</button></div></div><div class="profile-body"><div><h3>About ${esc(v.name)}</h3><p>${esc(v.about)}</p></div><div class="facts"><span>☀ Best time<br><b>${esc(v.best)}</b></span><span>⌖ How to reach<br><b>${esc(v.reach)}</b></span><span>◉ Languages<br><b>${esc(v.language)}</b></span></div></div><div class="profile-tabs" id="profile-tabs"><button class="profile-tab active" data-p="1">1 · Overview</button><button class="profile-tab" data-p="2">2 · Explore</button><button class="profile-tab" data-p="3">3 · Experiences</button><button class="profile-tab" data-p="4">4 · Food</button><button class="profile-tab" data-p="5">5 · People & Stories</button><button class="profile-tab" data-p="6">6 · Stay & Wedding</button></div><div id="profile-pages">${profilePage(1,v)}</div>`}
function ensureMobileModalStyles(){
 if(document.getElementById('villagedeko-mobile-modal-runtime-styles'))return;
 const s=document.createElement('style');s.id='villagedeko-mobile-modal-runtime-styles';s.textContent=`
 .modal-box{display:flex;flex-direction:column;max-height:calc(100svh - 24px);overflow:hidden;padding:24px}
 #modal-content{display:flex;flex-direction:column;min-height:0;max-height:100%;overflow:hidden}
 .modal-scroll{min-height:0;overflow-y:auto;overflow-x:hidden;padding-right:2px;-webkit-overflow-scrolling:touch}
 .modal-footer{flex:0 0 auto;padding-top:10px;background:#fffdf8}
 .modal-footer .primary{margin-top:0}
 @media(max-width:640px){
   .modal{padding:8px;align-items:center}
   .modal-box{width:min(100%,520px);max-height:calc(100svh - 16px);padding:18px;border-radius:18px}
   .modal-box h3{font-size:24px;line-height:1.12;margin:0 34px 10px 0}
   .modal-box p{font-size:13px;line-height:1.5}
   .modal-footer{padding-top:8px}
   .modal-footer .primary{padding:9px 14px;font-size:13px}
   .close{right:10px;top:10px;width:34px;height:34px;font-size:21px}
   .user-profile-card{gap:10px;padding:12px;border-radius:15px;margin:6px 0 10px}
   .user-profile-photo{width:58px;height:58px}
   .user-profile-card h3{font-size:24px!important;margin:0 28px 5px 0!important}
   .user-profile-stats{gap:6px;margin:10px 0 12px}
   .user-profile-stats>div{padding:8px;border-radius:12px}
   .user-profile-stats b{font-size:15px}
   .user-profile-stats small{font-size:10px;line-height:1.2}
   .saved-village-list{max-height:150px}
   .host-dashboard-grid{gap:7px;margin:10px 0 12px}
   .host-dashboard-grid>div{padding:10px;border-radius:12px}
   .menu-actions{gap:7px;margin:12px 0}
   .menu-actions button{padding:10px 13px}
   .food-journey-modal .journey{padding:12px;gap:5px;font-size:12px}
   .story-reader img{max-height:36vh;object-fit:cover;border-radius:14px}
 }
 `;document.head.appendChild(s);
}
ensureMobileModalStyles();
