...
function ensureVillageFeedStyles(){
 ...
 .village-feed-media,.village-feed-video{width:100%;aspect-ratio:4/3;object-fit:cover;display:block;background:#eee}.village-feed-video{background:#111}
 .village-feed-photo-btn{display:block;width:100%;padding:0;border:0;background:#eee;cursor:zoom-in}.village-feed-photo-btn:focus-visible{outline:3px solid #8a6b2f;outline-offset:-3px}
 .village-feed-body{padding:18px}.village-feed-meta{display:flex;justify-content:space-between;gap:12px;align-items:flex-start;margin-bottom:10px}.village-feed-author{min-width:0}.village-feed-village-link{display:block;border:0;background:none;padding:0;margin:0 0 3px;color:#6f5728;font:800 17px/1.25 inherit;cursor:pointer;text-align:left}.village-feed-village-link:hover{text-decoration:underline}.village-feed-meta strong{display:block;font-size:14px;color:#333}.village-feed-delete{border:0;background:transparent;color:#777;font-size:24px;line-height:1;padding:2px 6px;cursor:pointer}.village-feed-delete:hover{color:#222}.village-feed-body h3{margin:0 0 8px;font-size:22px}.village-feed-body p{margin:0;color:#555;line-height:1.6}.village-feed-actions{display:flex;align-items:center;gap:6px;margin-top:16px}.feed-icon-btn{width:40px;height:40px;display:grid;place-items:center;border:0;background:transparent;border-radius:50%;cursor:pointer;color:#444;transition:background .18s ease,transform .18s ease,color .18s ease}.feed-icon-btn:hover{background:#f2efe8;transform:translateY(-1px)}.feed-icon-btn.is-active{color:#8a6b2f;background:#f3eee3}.vd-feed-icon{display:grid;place-items:center}.vd-feed-icon svg{width:21px;height:21px;fill:none;stroke:currentColor;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round}.feed-icon-btn[data-action="feed-like"].is-active .vd-feed-icon svg{fill:currentColor}.feed-icon-btn[data-action="feed-save"].is-active .vd-feed-icon svg{fill:currentColor}.village-feed-empty{max-width:760px;margin:0 auto}.feed-host-note{padding:12px 14px;border-radius:14px;background:#f3f0e9;margin:12px 0;font-size:13px}.vd-feed-form{display:grid;gap:14px}.vd-feed-form .vd-feature-card{padding:14px 16px}.vd-feed-form label{display:grid;gap:7px;font-weight:700;font-size:13px}.vd-feed-form input,.vd-feed-form textarea{width:100%;box-sizing:border-box;border:1px solid rgba(0,0,0,.14);border-radius:12px;padding:11px 12px;background:#fff;font:inherit;font-size:16px}.vd-feed-form textarea{resize:vertical;min-height:120px}.vd-feed-form input[type=file]{padding:10px}.vd-feed-form .menu-actions{margin-top:4px}
 .village-feed-photo-viewer{position:fixed;inset:0;z-index:1100;background:rgba(10,10,10,.96);display:none;align-items:center;justify-content:center;padding:20px;box-sizing:border-box}.village-feed-photo-viewer.show{display:flex}.village-feed-photo-viewer img{max-width:94vw;max-height:88vh;width:auto;height:auto;object-fit:contain;display:block}.village-feed-photo-close{position:absolute;top:max(16px,env(safe-area-inset-top));right:max(16px,env(safe-area-inset-right));width:46px;height:46px;border:0;border-radius:50%;background:rgba(255,255,255,.14);color:#fff;font-size:30px;line-height:1;cursor:pointer}.village-feed-photo-title{position:absolute;left:50%;bottom:max(18px,env(safe-area-inset-bottom));transform:translateX(-50%);max-width:80vw;padding:7px 12px;border-radius:999px;background:rgba(0,0,0,.45);color:#fff;font-size:13px;font-weight:700;text-align:center}
 ...
 @media(max-width:640px){.village-feed-section{padding:42px 18px 58px}.village-feed-body{padding:15px}.village-feed-body h3{font-size:19px}.village-feed-village-link{font-size:16px}.feed-icon-btn{width:38px;height:38px}.profile-feed-card{grid-template-columns:1fr}.profile-feed-card img,.profile-feed-card video{width:100%;height:180px}.village-feed-photo-viewer{padding:12px}.village-feed-photo-viewer img{max-width:96vw;max-height:84vh}}
...
function feedIcon(name,label){
 const icons={
  ...
  share:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 16V4m0 0 4 4m-4-4L8 8"/><path d="M5 12v7a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-7"/></svg>'
 };
 return `<span class="vd-feed-icon">${icons[name]||''}</span><span class="sr-only">${esc(label)}</span>`;
}
function renderVillageFeed(){
 ...
 root.innerHTML=posts.map(p=>{
  ...
  const mediaHtml=media?(isVideo?`<video class="village-feed-video" controls playsinline preload="metadata"><source src="${esc(media)}">Your browser does not support video playback.</video>`:`<button class="village-feed-photo-btn" data-action="feed-photo" data-post-id="${esc(p.id)}" aria-label="Open full photo"><img class="village-feed-media" src="${esc(media)}" alt="${esc(p.title||'Village update')}" loading="lazy"></button>`):'';
  const villageId=p.villageId||p.ownerUid||'';
  const villageName=p.villageName||'Village';
  const likeKey=firebaseUser?`villagedeko_feed_likes_${firebaseUser.uid}`:'';
  const liked=firebaseUser&&vdUserArray(likeKey).includes(p.id);
  const saved=firebaseUser&&vdUserArray('villagedeko_saved_feed_posts').some(x=>x.id===p.id);
  return `<article class="village-feed-card"><div class="village-feed-body"><div class="village-feed-meta"><div class="village-feed-author"><button class="village-feed-village-link" data-action="feed-village" data-village-id="${esc(villageId)}" type="button">📍 ${esc(villageName)}</button><strong>${esc(p.authorName||'Village Host')}</strong></div>${firebaseUser&&p.ownerUid===firebaseUser.uid?`<button class="village-feed-delete" data-action="delete-feed-post" data-post-id="${esc(p.id)}" type="button" aria-label="Delete post">×</button>`:''}</div>${mediaHtml}<h3>${esc(p.title||'Village update')}</h3><p>${esc(p.text||'')}</p><div class="village-feed-actions" role="group" aria-label="Post actions"><button class="feed-icon-btn ${liked?'is-active':''}" data-action="feed-like" data-post-id="${esc(p.id)}" aria-label="Like" aria-pressed="${liked}">${feedIcon('like','Like')}</button><button class="feed-icon-btn" data-action="feed-comment" data-post-id="${esc(p.id)}" aria-label="Comment">${feedIcon('comment','Comment')}</button><button class="feed-icon-btn ${saved?'is-active':''}" data-action="feed-save" data-post-id="${esc(p.id)}" aria-label="Save" aria-pressed="${saved}">${feedIcon('save','Save')}</button><button class="feed-icon-btn" data-action="feed-share" data-post-id="${esc(p.id)}" aria-label="Share">${feedIcon('share','Share')}</button></div></div></article>`;
 }).join('');
}
function openFeedPhoto(id){const p=findFeedPost(id),url=p?.media?.url;if(!url)return;let v=document.getElementById('village-feed-photo-viewer');if(!v){v=document.createElement('div');v.id='village-feed-photo-viewer';v.className='village-feed-photo-viewer';v.innerHTML='<button type="button" class="village-feed-photo-close" aria-label="Close photo">×</button><img alt=""><div class="village-feed-photo-title"></div>';document.body.appendChild(v);v.querySelector('.village-feed-photo-close').addEventListener('click',()=>v.classList.remove('show'));v.addEventListener('click',e=>{if(e.target===v)v.classList.remove('show')});}v.querySelector('img').src=url;v.querySelector('img').alt=p.title||'Village photo';v.querySelector('.village-feed-photo-title').textContent=p.villageName||'Village';v.classList.add('show');}
