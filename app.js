...
const products=[['Handmade Pottery','Made by local artisans','650','hero-cows.jpg','Handicrafts'],['Local Textiles','Village-made craft','750','hero-cows.jpg','Textiles'],['Organic Honey','From village farms','550','hero-cows.jpg','Honey']];
...
function renderMarket(category='all'){const cats=['all','Food & Grains','Honey','Spices','Handicrafts','Local Art','Textiles','Handmade Products'];const cr=document.getElementById('market-categories');if(cr)cr.innerHTML=cats.map(c=>`<button class="market-category ${c===category?'active':''}" data-action="market-category" data-category="${c}">${c==='all'?'All Products':c}</button>`).join('');const grid=document.getElementById('market-grid');if(!grid)return;const list=category==='all'?products.map((p,i)=>[p,i]):products.map((p,i)=>[p,i]).filter(([p])=>p[4]===category);grid.innerHTML=list.map(([p,i])=>`<article><img src="${p[3]}"><h3>${p[0]}</h3><p>${p[1]} · ₹${p[2]}</p><button data-action="product" data-product="${i}">View Product</button></article>`).join('')}
