...
function renderStates(){document.getElementById('state-slider').innerHTML=`<button class="state-card active" data-state="all"><span class="state-no">ALL INDIA</span><h3>All India</h3><p>${states.length} regions</p></button>`+states.map((s,i)=>`<button class="state-card" data-state="${s}"><h3>${s}</h3><p>Explore villages</p></button>`).join('')}
