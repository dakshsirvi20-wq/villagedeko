...
function makeGenericStateVillage(state){
 const slug=String(state||'').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'');
 return {id:`generic-${slug}`,name:`Village Life in ${state}`,district:`${state} · Generic Preview`,state,cover:'hero-cows.jpg',about:`A generic VillageDeko preview for ${state}. Real villages from this state will appear here as local hosts publish their villages.`,best:'Explore local seasons',reach:'Local travel details will be added with village listings',language:'Local language · Hindi',generic:true};
}
function renderVillages(state='all'){if(firebaseUser)syncSavedListingIntoVillages();const unique=dedupeVillagesByIdentity(villages);if(unique.length!==villages.length)villages.splice(0,villages.length,...unique);const uiState=document.getElementById('state-filter')?.value||'all';if(uiState!=='all'&&uiState!==state)state=uiState;selectedState=state;const {search}=getVillageFilters();let list=state==='all'?villages:villages.filter(v=>v.state===state);if(state!=='all'&&!list.length)list=[makeGenericStateVillage(state)];if(search)list=list.filter(v=>`${v.name} ${v.district} ${v.state}`.toLowerCase().includes(search));document.getElementById('selected-state-label').textContent=state==='all'?'All India':state;const empty=document.getElementById('empty-state');empty.hidden=list.length>0;empty.innerHTML='';document.getElementById('village-grid').innerHTML=list.map(v=>`<article class="village-card" data-action="village" data-id="${v.id}" role="button" tabindex="0" aria-label="Open ${v.name} village profile"><img src="${v.cover}" alt="${v.name}"><div><span>${v.generic?'GENERIC PREVIEW':v.state}</span><h3>${v.name}</h3><p>${v.district} · ${v.state}</p><button type="button" data-action="village" data-id="${v.id}">Explore Village</button></div></article>`).join('');document.querySelectorAll('.state-card').forEach(x=>x.classList.toggle('active',x.dataset.state===state));const sf=document.getElementById('state-filter');if(sf)sf.value=state;document.getElementById('village-grid').scrollLeft=0}
...
function ensureVillageFeedStyles(){
 ...
 .village-feed-media-wrap{width:100%;aspect-ratio:4/5;overflow:hidden;background:#eee;position:relative}.village-feed-media,.village-feed-video{width:100%;height:100%;aspect-ratio:auto;object-fit:cover;display:block;background:#eee}.village-feed-video{background:#111}
