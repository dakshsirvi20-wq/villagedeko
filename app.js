...
async function persistVillageToFirebase(data){
 ...
 // Upload per-experience photos and keep their URL inside the experience row.
 if(Array.isArray(data.experience)){
  const experienceCards=[...document.querySelectorAll('#experience-repeat .repeat-card')];
  for(let i=0;i<data.experience.length;i++){
   const fileInput=experienceCards[i]?.querySelector('input[type="file"]');
   const file=fileInput?.files?.[0];
   if(file){
    const uploaded=await reuseOrUploadCloudinary(file,[]);
    if(uploaded?.url)data.experience[i][5]=uploaded.url;
   }else if(fileInput?.dataset.existing){
    data.experience[i][5]=fileInput.dataset.existing;
   }
  }
 }
...
const stories=[
{villageId:'kuthar',title:'40 years of farming, one field at a time.',tag:'Village Life',category:'village-life',person:'Gopal Singh · Farmer',text:'Gopal shares how one family field changed through four decades — from seed and water to harvest and mandi.',img:'hero-cows.jpg'},
{villageId:'kuthar',title:'The recipe our village remembers.',tag:'Local Food Story',category:'local-food',person:'Sita Devi · Local Cook',text:'A traditional meal, cooked slowly and explained by the person who has carried the recipe through generations.',img:'hero-cows.jpg'},
{villageId:'kuthar',title:'Clay, hands and a living craft.',tag:'People & Culture',category:'people-culture',person:'Hukamram · Potter',text:'Meet a village artisan and understand how local clay becomes everyday objects and keeps an old skill alive.',img:'hero-cows.jpg'},
{villageId:'kuthar',title:'The traditions that bring the village together.',tag:'Festivals / Traditions',category:'festivals-traditions',person:'Village Elders · Community Story',text:'A look at the shared customs, music, food and gatherings that keep village traditions alive across generations.',img:'hero-cows.jpg'},
{villageId:'punjab',title:'My field, my life, my story.',tag:'Gaon Ke Log',category:'gaon-ke-log',person:'Ramesh ji · Farmer',text:'A farmer explains what a normal day in the field looks like, why each season matters and what farming means to his family.',img:'hero-cows.jpg'},
{villageId:'punjab',title:'The animals are part of our family.',tag:'Gaon Ke Log',category:'gaon-ke-log',person:'Kamla Devi · Pashupalak',text:'A village livestock keeper shares the daily rhythm of feeding, caring for animals and the role dairy plays in village life.',img:'hero-cows.jpg'},
{villageId:'bhuj',title:'Keeping an old craft alive.',tag:'Gaon Ke Log',category:'gaon-ke-log',person:'Mohan ji · Local Artisan',text:'A local artisan shares how a traditional skill is learned, practiced and passed from one generation to the next.',img:'hero-cows.jpg'}
];
const experiences=[
...
const products=[
 {name:'Bajra & Village Grains',price:320,image:'hero-cows.jpg',category:'Food & Grains',seller:'Gopal Singh · Farmer',village:'Kuthar Village · Jodhpur District · Rajasthan',story:'40 years of farming, one field at a time — a farmer story about seed, water, harvest and the life behind village grains.'},
 {name:'Raw Village Honey',price:550,image:'hero-cows.jpg',category:'Honey',seller:'Kuthar Village Farmer Collective',village:'Kuthar Village · Jodhpur District · Rajasthan',story:'Honey from village-side farming and local work, connected to the people and landscape that produce it.'},
 {name:'Hand-ground Red Chilli',price:280,image:'hero-cows.jpg',category:'Spices',seller:'Kuthar Village Women Producers',village:'Kuthar Village · Jodhpur District · Rajasthan',story:'A local spice story that connects everyday cooking with the ingredients grown, prepared and used in the village.'},
 {name:'Handmade Pottery',price:650,image:'hero-cows.jpg',category:'Handicrafts',seller:'Hukamram · Potter',village:'Kuthar Village · Jodhpur District · Rajasthan',story:'Clay, hands and a living craft — understand how local clay becomes everyday objects and keeps an old skill alive.'},
 {name:'Hand-painted Folk Art',price:900,image:'hero-cows.jpg',category:'Local Art',seller:'Mohan ji · Local Artisan',village:'Bhujodi Village · Kutch District · Gujarat',story:'Keeping an old craft alive — a local artisan story about learning, practicing and passing a traditional skill forward.'},
 {name:'Handwoven Village Textile',price:750,image:'hero-cows.jpg',category:'Textiles',seller:'Bhujodi Artisan Collective',village:'Bhujodi Village · Kutch District · Gujarat',story:'Village-made textile work that carries local craft knowledge from the artisan’s hands into everyday life.'},
 {name:'Handcrafted Utility Basket',price:450,image:'hero-cows.jpg',category:'Handmade Products',seller:'Kuthar Craft Collective',village:'Kuthar Village · Jodhpur District · Rajasthan',story:'A useful handmade object with a village story — made through local skill, time and generations of craft knowledge.'}
];
...
async function saveBazaarProduct(){
 ...
 try{
  const file=f.elements.productImage?.files?.[0];let image='hero-cows.jpg';
...
function renderStates(){const fallback='hero-cows.jpg';const stateCover=s=>villages.find(v=>v.state===s&&v.cover)?.cover||fallback;document.getElementById('state-slider').innerHTML=`<button class="state-card active state-card-all" data-state="all"><span class="state-card-shade"></span><img src="${fallback}" alt="All India"><span class="state-card-copy"><small>ALL INDIA</small><strong>All India</strong><em>${states.length} regions</em></span></button>`+states.map(s=>`<button class="state-card" data-state="${s}"><span class="state-card-shade"></span><img src="${stateCover(s)}" alt="${esc(s)}"><span class="state-card-copy"><strong>${esc(s)}</strong></span></button>`).join('')}
...
function listingToVillage(d){
 ...
 listing.experience=Array.isArray(d.experience)?d.experience.map(row=>{
  if(!Array.isArray(row))return row;
  return [row[0]||'activities',row[1]||'Experience',row[4]||row[2]||'',row[3]||'0',row[5]||'hero-cows.jpg'];
 }):[];
 return {
...
const listingTemplates={
 ...
 experience:[['Experience name','text','e.g. Farm Visit'],['Category','select',['Activities','Food','Culture','Nature','Farming','Craft','Other']],['Duration','text','e.g. 2 Hours'],['Price per person','number','₹'],['Description','textarea','What will visitors do?'],['Experience photo','file','']],
...
function renderListingProgress(){const root=document.getElementById('listing-progress');if(!root)return;const names=['Overview','Identity','Explore','Experiences','Ek Din','Food','People & Stories','Culture','Visit Info','Stay & Wedding','Review'];root.innerHTML=names.map((n,i)=>`<button type="button" class="${i+1===listingFormState.step?'active':''}" data-listing-step="${i+1}">${n}</button>`).join('')}
...
function fillListingFormFromSaved(d){
 ...
 Object.keys(d).forEach(k=>{if(listingTemplates[k]||k==='coverPhoto'||k==='galleryPhotos'||k==='galleryVideos'||k==='stayPhoto')return;const el=f.elements[k];if(el&&typeof d[k]==='string')el.value=d[k]});
...
function renderVillageBookingSummary(){
 ...
 if(!root||!t)return;root.innerHTML=`<div><span>Package</span><b>${esc(t.name)}</b></div><div><span>Stay</span><b>${esc(t.stay)}</b></div><div><span>Host</span><b>${esc(t.host)}</b></div><div><span>Listed price</span><b>₹${Number(t.price||0).toLocaleString('en-IN')}</b></div><div><span>Date</span><b>${d?.value||'Select a date'}</b></div><div><span>Guests</span><b>${Number(g?.value||2)}</b></div><p>Final availability and any transport/details will be confirmed by the host.</p>`;
}
async function submitVillageBooking(){
 ...
 const guests=Math.max(1,Math.min(20,Number(document.getElementById('vd-booking-guests')?.value||2)));const ownerUid=v.ownerUid||v.listing?.ownerUid||'';const id=`${firebaseUser.uid}_${Date.now()}`;
 const booking={id,uid:firebaseUser.uid,userName:firebaseUser.displayName||'VillageDeko User',userEmail:firebaseUser.email||'',ownerUid,villageId:v.id,villageName:v.name,district:v.district,state:v.state,tripIndex:i,trip:t.name,duration:t.duration,stay:t.stay,price:Number(t.price||0),guests,date,note:document.getElementById('vd-booking-note')?.value?.trim()||'',status:'pending',createdAt:Date.now()};
 const local=[...vdUserArray('villagedeko_bookings'),booking];vdSetUserArray('villagedeko_bookings',local);let savedRemote=false;
