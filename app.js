...
async function initFirebase(){
 try{
  ...
  onAuthStateChanged(firebaseAuth, async user=>{ firebaseUser=user||null; updateAuthUI(); if(user) await loadRemoteListing(); });
...
async function getCloudinaryFileKey(file){
 if(!file||!file.size)return null;
 const buffer=await file.arrayBuffer();
 const hash=await crypto.subtle.digest('SHA-256',buffer);
 return Array.from(new Uint8Array(hash)).map(b=>b.toString(16).padStart(2,'0')).join('');
}
function getCloudinaryUploadCache(){
 try{return JSON.parse(localStorage.getItem('villagedeko_cloudinary_upload_cache')||'{}')||{};}catch{return {};}
}
function setCloudinaryUploadCache(cache){
 try{localStorage.setItem('villagedeko_cloudinary_upload_cache',JSON.stringify(cache));}catch{}
}
async function uploadCloudinary(file){
 const cfg=await import('./villagedeko-config.js');
 if(!file||!file.size)return null;
 const key=await getCloudinaryFileKey(file);
 const cache=getCloudinaryUploadCache();
 if(key&&cache[key]?.publicId)return cache[key];
 ...
 const uploaded={url:j.secure_url,publicId:j.public_id,resourceType:j.resource_type,width:j.width||null,height:j.height||null,bytes:j.bytes||null,format:j.format||null};
 if(key){cache[key]=uploaded;setCloudinaryUploadCache(cache);}
 return uploaded;
}
function mergeCloudinaryMedia(existing, incoming){
 const oldList=Array.isArray(existing)?existing:[];
 const newList=Array.isArray(incoming)?incoming:[];
 const seen=new Set();
 return [...oldList,...newList].filter(item=>{
  const id=item?.publicId||item?.url;
...
async function persistVillageToFirebase(data){
 ...
 const files={}; const f=document.getElementById('village-listing-form');
 ...
 if(cover) files.cover=await uploadCloudinary(cover);
 if(photos.length) files.photos=await Promise.all(photos.map(uploadCloudinary));
 if(videos.length) files.videos=await Promise.all(videos.map(uploadCloudinary));
 const existing=JSON.parse(localStorage.getItem('villagedeko_my_village_listing')||'null')||{};
 data.media={
  cover:files.cover||existing.media?.cover||null,
  photos:mergeCloudinaryMedia(existing.media?.photos,files.photos),
  videos:mergeCloudinaryMedia(existing.media?.videos,files.videos)
 };
...
function syncSavedListingIntoVillages(){
 ...
 if(i>=0)villages[i]=saved;else villages.unshift(saved);
}
...
function deleteVillageListing(){if(!localStorage.getItem('villagedeko_my_village_listing')){openModal('No saved listing','There is no saved village listing on this device yet.');return}if(confirm('Delete your saved VillageDeko listing? This cannot be undone.')){if(firebaseUser&&firestoreDb){window.VD_FIREBASE.deleteDoc(window.VD_FIREBASE.doc(firestoreDb,'villages',firebaseUser.uid)).catch(console.warn)}localStorage.removeItem('villagedeko_my_village_listing');localStorage.removeItem('villagedeko_cloudinary_upload_cache');document.getElementById('village-listing-form')?.reset();for(const type of Object.keys(listingTemplates)){const root=document.getElementById(`${type}-repeat`);if(root)root.innerHTML=''}ensureListingRows();goListingStep(1);openModal('Village Listing Deleted','Your saved village listing has been removed from this device.');}}
