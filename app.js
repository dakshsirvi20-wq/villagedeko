...
function getVillageDaySchedule(){
 const rows=currentVillage?.listing?.day;
 if(Array.isArray(rows)&&rows.length){
  return rows.filter(x=>Array.isArray(x)&&x.some(Boolean)).map(x=>{
   const activity=String(x[1]||'Village moment');
   const lower=activity.toLowerCase();
   const icon=lower.includes('dairy')||lower.includes('pashu')||lower.includes('doodh')?'🐄':lower.includes('khet')||lower.includes('farming')||lower.includes('fasal')?'🌾':lower.includes('chai')||lower.includes('nashta')?'☕':lower.includes('khana')||lower.includes('food')||lower.includes('rasoi')?'🍲':lower.includes('mandir')||lower.includes('bhajan')?'🛕':lower.includes('raat')?'🌙':lower.includes('subah')||lower.includes('morning')?'🌅':lower.includes('shaam')||lower.includes('evening')?'🏡':'🌿';
   return [String(x[0]||''),activity,String(x[2]||''),icon];
  });
 }
 return ekDinGaonMein;
}
function renderEkDinGaonMein(){
 const root=document.getElementById('experience-list'); if(!root)return;
 const schedule=getVillageDaySchedule();
 if(!schedule.length){root.innerHTML='<div class="empty-profile-data"><strong>Ek Din Gaon Mein abhi add nahi hua.</strong><p>Village owner listing form se time slots add kar sakta hai.</p></div>';return;}
 const custom=Array.isArray(currentVillage?.listing?.day)&&currentVillage.listing.day.length;
 root.innerHTML=`<article class="ek-din-card"><div class="ek-din-head"><div><span class="eyebrow">EK DIN GAON MEIN${custom?' · VILLAGE LISTING':''}</span><h3>Gaon ka ek poora din</h3><p>${custom?'Is village ke host ne apna real day schedule add kiya hai.':'Subah ki pehli roshni se gaon ki raat tak — ek din ko step by step dekho, samjho aur experience karo.'}</p></div><span class="ek-din-time">${schedule[0][0]} → ${schedule[schedule.length-1][0]}</span></div><div class="ek-din-timeline">${schedule.map((x,i)=>`<button class="ek-din-slot${i===0?' active':''}" data-action="ek-din-slot" data-ekdin="${i}"><span class="ek-din-icon">${x[3]}</span><small>${esc(x[0])}</small><strong>${esc(x[1])}</strong><span class="ek-din-arrow">→</span></button>`).join('')}</div><div class="ek-din-detail" id="ek-din-detail"><span>${esc(schedule[0][0])}</span><h4>${esc(schedule[0][1])}</h4><p>${esc(schedule[0][2])}</p><b>📍 ${esc(currentVillage.name)} · ${esc(currentVillage.district)} · ${esc(currentVillage.state)}</b></div></article>`;
}
...
function profilePage(n,v){
 ...
 if(n===3){const rows=d.experience||[],schedule=getVillageDaySchedule();const dayBlock=schedule.length?`<section class="ek-din-card profile-ek-din"><div class="ek-din-head"><div><span class="eyebrow">EK DIN GAON MEIN · VILLAGE LISTING</span><h3>Is gaon mein ek din</h3><p>Host ke listing form mein diye gaye morning-to-night time slots yahan directly dikh rahe hain.</p></div><span class="ek-din-time">${esc(schedule[0][0])} → ${esc(schedule[schedule.length-1][0])}</span></div><div class="ek-din-timeline">${schedule.map((x,i)=>`<button class="ek-din-slot${i===0?' active':''}" data-action="ek-din-slot" data-ekdin="${i}"><span class="ek-din-icon">${x[3]}</span><small>${esc(x[0])}</small><strong>${esc(x[1])}</strong><span class="ek-din-arrow">→</span></button>`).join('')}</div><div class="ek-din-detail" id="ek-din-detail"><span>${esc(schedule[0][0])}</span><h4>${esc(schedule[0][1])}</h4><p>${esc(schedule[0][2])}</p><b>📍 ${esc(v.name)} · ${esc(v.district)} · ${esc(v.state)}</b></div></section>`:`<section class="empty-profile-data"><strong>Ek Din Gaon Mein abhi add nahi hua.</strong><p>Village owner listing form mein time slots add karega to yahan automatically dikh jayenge.</p></section>`;return common+`<span class="eyebrow">EXPERIENCES</span><h3>Experiences & Activities</h3>${dayBlock}<h4 class="profile-subheading">Village Experiences</h4><div class="experience-list">${rows.length?rows.map((x,i)=>`<article class="experience profile-form-experience" data-action="listing-experience" data-index="${i}"><div><span class="experience-kicker">${esc(x[1]||'EXPERIENCE')}</span><h3>${esc(x[0]||'Experience')}</h3><p>${esc(x[4]||'')}</p><small>${esc(x[2]||'')}</small></div><strong>₹${esc(x[3]||'0')} <small>/ person</small></strong></article>`).join(''):`<div class="empty-profile-data"><strong>No experiences added yet.</strong><p>Add activities from the Village Listing form.</p></div>`}</div></div>`}
...
function renderExperiences(filter='all'){
 ...
 const daySchedule=getVillageDaySchedule();const ek=filter==='all'?`<article class="experience ek-din-experience-card" data-type="ek-din" data-action="filter-ek-din" role="button" tabindex="0"><div class="ek-din-card-icon">🌅</div><div><span class="experience-kicker">EK DIN GAON MEIN</span><h3>Gaon ka ek poora din</h3><p>${esc(daySchedule[0]?.[0]||'05:30 AM')} → ${esc(daySchedule[daySchedule.length-1]?.[0]||'09:00 PM')} · Is village ka day schedule step by step dekho.</p></div><strong>${daySchedule.length} moments</strong><button data-action="filter-ek-din">Explore</button></article>`:'';
...
function useRecommendedDaySchedule(){const root=document.getElementById('day-repeat');if(!root)return;root.innerHTML='';ekDinGaonMein.forEach(row=>addListingRow('day',row.slice(0,3)));renderListingReview();openModal('Recommended day added','10 moments add ho gaye hain. Aap har time, activity aur description ko apne real village ke hisaab se edit kar sakte hain.');}
function initListingForm(){if(!document.getElementById('village-listing-form'))return;populateListingState();ensureListingRows();fillListingFormFromSaved(getSavedListing());renderListingProgress();goListingStep(1);const rb=document.getElementById('use-recommended-day');if(rb&&!rb.dataset.bound){rb.dataset.bound='1';rb.addEventListener('click',useRecommendedDaySchedule)}listingFormState.initialized=true}
...
function ensureVillageFeedStyles(){
 ...
 @media(max-width:640px){.village-feed-section{padding:42px 18px 58px}.village-feed-body{padding:15px}.village-feed-body h3{font-size:19px}.village-feed-actions{gap:6px}.village-feed-actions button{padding:8px 11px;font-size:12px}.village-feed-media,.village-feed-video{aspect-ratio:4/5}}
...
/* VillageDeko bottom navigation: subtle transparent glass, keep existing layout. */
(function(){
  if(document.getElementById('vd-transparent-nav-style')) return;
  const st=document.createElement('style');
  st.id='vd-transparent-nav-style';
  st.textContent=`
    .bottom-nav{
      background:rgba(255,253,248,.82) !important;
      -webkit-backdrop-filter:blur(10px);
      backdrop-filter:blur(10px);
      border-top:1px solid rgba(72,83,64,.14) !important;
    }
    .nav-item{color:#596057 !important;}
    .nav-item span{color:#596057 !important;}
    .nav-item.active,.nav-item.active span{color:var(--green) !important;}
  `;
  document.head.appendChild(st);
})();
