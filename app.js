...
async function uploadToStorage(file, folder="uploads"){
  ...
  const path=`${folder}/${currentUser.uid}/${Date.now()}_${Math.random().toString(36).slice(2,8)}_${safeName}`;
  const storageRef=ref(storage,path);
  await uploadBytes(storageRef,file,{contentType:file.type||undefined});
  return await getDownloadURL(storageRef);
}
async function uploadToStorage(file,folder="uploads"){ return uploadToStorage(file,folder); }
