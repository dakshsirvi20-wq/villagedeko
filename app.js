...
function showView(id){ ["homeView","exploreView","experienceView","visitView","profileView","stateView","villageView","weddingView","chaupalView"].forEach(v=>$(v)?.classList.add("hidden")); $(id)?.classList.remove("hidden"); window.scrollTo({top:0,behavior:"smooth"}); }
...
function renderProfile(){
  ...
  c.innerHTML=`<div class="profile-card"><div class="profile-avatar">${esc((name||"U").charAt(0).toUpperCase())}</div><div><h2>${esc(name)}</h2><p>${esc(email)}</p><div class="profile-stats"><span><b>${mine}</b> posts</span><span><b>${listed}</b> villages</span></div></div></div><div class="profile-actions"><button onclick="openPostModal()" class="profile-action">📸 Create Post</button><button onclick="openModal('listVillageModal')" class="profile-action">🏡 List Village</button><button onclick="openChaupal()" class="profile-action">🔥 Chaupal</button><button onclick="openSettings()" class="profile-action">⚙️ Settings</button></div>`;
}
...
async function uploadToCloudinary(file, folder="uploads") {
  ...
  if (!CLOUDINARY_CLOUD_NAME || !CLOUDINARY_UPLOAD_PRESET) throw new Error("Cloudinary configuration missing hai.");
  const fd = new FormData();
  fd.append("file", file);
  fd.append("upload_preset", CLOUDINARY_UPLOAD_PRESET);
  fd.append("folder", `villagedeko/${folder}/${uid()}`);
  const response = await fetch(`https://api.cloudinary.com/v1_1/${encodeURIComponent(CLOUDINARY_CLOUD_NAME)}/image/upload`, { method: "POST", body: fd });
