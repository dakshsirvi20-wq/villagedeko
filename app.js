 ...
 try{
  ...
  snap.forEach(docSnap=>{
   ...
   if(village){
    ...
    village.feedPosts=Array.isArray(raw.feedPosts)?raw.feedPosts:[];
...
function syncSavedListingIntoVillages(){
 ...
 const normalized={...saved,id:'my-listing',ownerUid:firebaseUser?.uid||saved.ownerUid,feedPosts:Array.isArray(saved.feedPosts)?saved.feedPosts:[]};
 if(existingIndex>=0){
...
async function openTripPlanner(){
 ...
 openModal('Plan Your Village Trip',`<div class="vd-feature-card"><strong>${esc(v.name)}</strong><small>${esc(v.district)} · ${esc(v.state)}</small></div><label>Trip package<select id="vd-trip-select">${trips.map((t,i)=>`<option value="${i}">${esc(t.name)} · ${esc(t.duration)} · ₹${Number(t.price||0).toLocaleString('en-IN')}</option>`).join('')}</select></label><label>Visit date<input id="vd-trip-date" type="date"></label><label>Guests<input id="vd-trip-guests" type="number" min="1" max="20" value="2"></label><label>Note<textarea id="vd-trip-note" rows="3" placeholder="What would you like to experience?"></textarea></label><div class="menu-actions"><button class="primary" data-action="save-trip-plan">Save Trip Plan</button></div>`);
}
...
async function sendVillageEnquiry(){
 ...
 openModal('Enquire About This Village',`<div class="vd-feature-card"><strong>${esc(v.name)}</strong><small>Ask the host about visiting, stay or experiences.</small></div><label>Message<textarea id="vd-enquiry-message" rows="4" placeholder="I would like to know about..."></textarea></label><label>Preferred date<input id="vd-enquiry-date" type="date"></label><label>Guests<input id="vd-enquiry-guests" type="number" min="1" max="20" value="2"></label><div class="menu-actions"><button class="primary" data-action="send-enquiry" data-owner-uid="${esc(ownerUid)}">Send Enquiry</button></div>`);
}
...
async function openVillageFeedComposer(){if(!(await requireGoogleAuth()))return;if(getUserRole()!=='host'){openRoleChooser();return;}const listing=getHostListingForCurrentUser();if(!listing?.villageName){openModal('Village Listing Required','Feed post karne ke liye pehle apna Village Listing publish/save karo.');return;}openModal('Post to Village Feed',`<div class="vd-feature-card"><strong>${esc(listing.villageName)}</strong><small>${esc(listing.district||'')} · ${esc(listing.state||'')}</small></div><p class="feed-host-note">Ye post automatically isi listed village ke profile mein bhi save hogi.</p><label>Post title<input id="vd-feed-title" maxlength="120" placeholder="e.g. Aaj sarson ki pehli katai"></label><label>Story / update<textarea id="vd-feed-text" rows="5" maxlength="1200" placeholder="Gaon mein aaj kya ho raha hai..."></textarea></label><label>Photo or video (optional)<input id="vd-feed-media" type="file" accept="image/*,video/*"></label><div class="menu-actions"><button class="primary" data-action="publish-feed-post">Publish to Village Feed</button></div>`);}
async function publishVillageFeedPost(){
 ...
 try{
  ...
  setLocalFeedPosts(firebaseUser.uid,[...posts,post]);
  const v=villages.find(x=>x.ownerUid===firebaseUser.uid||x.id==='my-listing');
